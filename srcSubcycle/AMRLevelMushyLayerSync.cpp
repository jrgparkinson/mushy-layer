...
void AMRLevelMushyLayer::doMomentumReflux(
    Vector<LevelData<FArrayBox> *> &compVel)
{
  ...
  Vector<LevelData<FArrayBox> *> refluxCorr(finest_level + 1, nullptr);
  // this is necessary because while solver only can do
  // one component, levelfluxRegister::reflux can only
  // do all of them at once.
  Vector<LevelData<FArrayBox> *> tempRefluxData(finest_level + 1, nullptr);
  ...
  for (int lev = startLev; lev <= finest_level; lev++)
  {
    ...
    refluxCorr[lev] = new LevelData<FArrayBox>(levelGrids, 1, ghostVect);
  ...
  Real velNorm =
      computeNorm(vectVel, AmrRefRatios, m_dx, allVelComps, normType, m_level);
  ...
  for (int dir = 0; dir < SpaceDim; dir++)
  {
    ...
    Real alpha = 1.0; // 1.0
    Real beta =
        -m_parameters.m_viscosityCoeff * m_dt; // -m_parameters.prandtl * m_dt;

    AMRPoissonOpFactory viscousOpFactory;
    viscousOpFactory.define(baseDomain, AmrGrids, AmrRefRatios, AmrDx[0],
                            m_physBCPtr->viscousRefluxBC(dir, m_opt.viscousBCs),
                            alpha, beta);
    ...
    viscousSolver.define(baseDomain, viscCastFact, &bottomSolver, numLevels);
    ...
    // now solve
    viscousSolver.solve(refluxCorr, refluxRHS, finest_level, m_level,
    ...
    for (int lev = finest_level; lev >= m_level; --lev)
    {
      ...
      for (levelDit.reset(); levelDit.ok(); ++levelDit)
      {
        ...
        //        (*amrMLptr->m_vectorNew[m_fluidRefluxCorr])[levelDit].plus(levelCorr[levelDit()],
        //        0, dir, 1);
      }
...
void AMRLevelMushyLayer::postTimeStep()
{
  ...
  if (m_level == 0)
  {
    ...
    if (timestepFailed)
    {
      ...
      if (m_dt / 2 < m_opt.minDt)
      {
        ...
        if (s_verbosity > 1)
        {
          LOG_INFO("postTimestep() - Not reducing dt as it would break the "
                   "minimum dt cap");
        }
      }
  ...
  if (hasFinerLevel())
  {
    ...
    for (int scalarVar = 0; scalarVar < m_numScalarVars; scalarVar++)
    {
      ...
      fineAMRMLPtr->m_coarseAverageScalar.averageToCoarse(
          *m_scalarNew[scalarVar], *(fineAMRMLPtr->m_scalarNew[scalarVar]));
    }

    for (int vectorVar = 0; vectorVar < m_numVectorVars; vectorVar++)
    {
      fineAMRMLPtr->m_coarseAverageVector.averageToCoarse(
          *m_vectorNew[vectorVar], *(fineAMRMLPtr->m_vectorNew[vectorVar]));
    }
    ...
    if (m_level == 0 || (abs(crseTime - m_time) > TIME_EPS))
    {
          ...
          m_opt.reflux_lambda)
      {
        ...
        if (m_level == 0)
        {
          ...
          Real maxLambda =
              ::computeNorm(amrLambda, nRef, m_dx, Interval(0, 0), 0, m_level) -
              1;
      ...
      Vector<RefCountedPtr<LevelData<FluxBox>>> compPorosityFace(finest_level +
                                                                 1);
      Vector<RefCountedPtr<LevelData<FArrayBox>>> compPorosity(finest_level +
                                                               1);

      if (m_level > 0)
      {
        // coarser level will be coarse-fine BC
        AMRLevelMushyLayer *crseAMRLevelMushyLayerPtr = getCoarserLevel();
        ...
        crseAMRLevelMushyLayerPtr->fillVectorField(*crseVelPtr, m_time,
                                                   m_fluidVel, true);
        compVel[m_level - 1] = crseVelPtr;

        LevelData<FArrayBox> *crseLambdaPtr =
            new LevelData<FArrayBox>(crseGrids, 1);
        crseAMRLevelMushyLayerPtr->fillScalars(*crseLambdaPtr, m_time, m_lambda,
                                               true);
        compLambda[m_level - 1] = crseLambdaPtr;

        RefCountedPtr<LevelData<FluxBox>> crsePorosityFacePtr =
            RefCountedPtr<LevelData<FluxBox>>(
                new LevelData<FluxBox>(crseGrids, 1));
        crseAMRLevelMushyLayerPtr->fillScalarFace(*crsePorosityFacePtr, m_time,
                                                  m_porosity, true);
        compPorosityFace[m_level - 1] = crsePorosityFacePtr;

        RefCountedPtr<LevelData<FArrayBox>> crsePorosityPtr =
            RefCountedPtr<LevelData<FArrayBox>>(
                new LevelData<FArrayBox>(crseGrids, 1));
        crseAMRLevelMushyLayerPtr->fillScalars(*crsePorosityPtr, m_time,
                                               m_porosity, true);
      ...
      for (int lev = m_level; lev <= finest_level; lev++)
      {
        ...
        RefCountedPtr<LevelData<FluxBox>> porosityFacePtr =
            RefCountedPtr<LevelData<FluxBox>>(
                new LevelData<FluxBox>(thisMLPtr->m_grids, 1));
        thisMLPtr->fillScalarFace(*porosityFacePtr, m_time, m_porosity, true);
        compPorosityFace[lev] = porosityFacePtr;

        RefCountedPtr<LevelData<FArrayBox>> porosityPtr =
            RefCountedPtr<LevelData<FArrayBox>>(
                new LevelData<FArrayBox>(thisMLPtr->m_grids, 1));
        ...
        velBC.applyBCs(*compVel[lev], levelGrids, thisMLPtr->problemDomain(),
      ...
      if (m_opt.refluxAverageDown)
      {
        ...
        for (int lev = finest_level - 1; lev >= m_level; --lev)
        {
          ...
          LevelData<FArrayBox> &fineVel =
              *fineML.m_vectorNew[VectorVars::m_fluidVel];
          avgDown.averageToCoarse(levelVel, fineVel);

          CoarseAverage &scalAvgDown = fineML.m_coarseAverageScalar;

          scalAvgDown.averageToCoarse(
              *thisMLPtr->m_scalarNew[ScalarVars::m_enthalpy],
              *fineML.m_scalarNew[ScalarVars::m_enthalpy]);
          scalAvgDown.averageToCoarse(
              *thisMLPtr->m_scalarNew[ScalarVars::m_bulkConcentration],
      ...
      {
        m_projection.doSyncOperations(compVel, compLambda, compPorosityFace,
                                      compPorosity, m_time, m_dt);
      }
  ...
  else
  {
    ...
    // If we've coarsened a previous  mesh, we may have some lambda error left
    ...
    if (m_level == 0 && m_opt.max_possible_level > 0)
    {
      ...
      velBC.applyBCs(*m_vectorNew[VectorVars::m_fluidVel], m_grids,
                     m_problem_domain, m_dx, false); // not homogeneous
  ...
  if (m_level == 0)
  {
    ...
    {
      DataIterator dit =
          (*AMRmlptr->m_vectorNew[VectorVars::m_fluidVelErr]).dataIterator();
      for (dit.reset(); dit.ok(); ++dit)
      {
        (*AMRmlptr->m_vectorNew[VectorVars::m_fluidVelErr])[dit].setVal(0);
        (*AMRmlptr->m_vectorNew[VectorVars::m_fluidVelErr])[dit].plus(
            (*AMRmlptr->m_vectorNew[VectorVars::m_fluidVel])[dit]);
        (*AMRmlptr->m_vectorNew[VectorVars::m_fluidVelErr])[dit].minus(
            (*AMRmlptr->m_vectorNew[VectorVars::m_fluidVelAnalytic])[dit]);

        (*AMRmlptr->m_scalarNew[ScalarVars::m_Terr])[dit].copy(
            (*AMRmlptr->m_scalarNew[ScalarVars::m_temperatureAnalytic])[dit]);
        (*AMRmlptr->m_scalarNew[ScalarVars::m_Terr])[dit].minus(
            (*AMRmlptr->m_scalarNew[ScalarVars::m_temperature])[dit]);
      }
      ...
      Real maxU =
          ::computeNorm(*(AMRmlptr->m_vectorNew[VectorVars::m_fluidVel]),
                        nullptr, 1, m_dx, Interval(0, SpaceDim - 1), 0);
      if (maxU > 1e10)
      {
  ...
  if (m_opt.computeDiagnostics && (m_level == 0 || printAllLevels))
  {

    // If a diagnostic period has been declared, check this time has passed
    // since we last produced diagnostics
    ...
    {
      m_diagnostics.addDiagnostic(DiagnosticNames::diag_timestep, m_time,
                                  AMR::s_step);
...
void AMRLevelMushyLayer::doExplicitReflux(int a_var)
{
  ...
  if (s_verbosity > 5)
  {
    pout() << "AMRLevelMushyLayer::doExplicitReflux, var = "
           << m_scalarVarNames[a_var] << ", level = " << m_level << endl;
  }
  int finest_level = getFinestLevel();
  ...
  for (int lev = startLev; lev <= finest_level; lev++)
  {
    ...
    //    fullRHS[lev] = new
    //    LevelData<FArrayBox>(phiNew[lev]->disjointBoxLayout(), 1);
  ...
  for (int lev = m_level; lev <= finest_level; lev++)
  {
    if (lev < finest_level)
    {
      ...
      thisMLPtr->m_fluxRegisters[a_var]->reflux(levelRefluxRHS, refluxScale);

      // initial guess for correction is RHS
      levelRefluxRHS.copyTo(solverComps, *(scalRefluxCorr[lev]), solverComps);
    }
  ...
  for (int lev = finest_level; lev >= m_level; lev--)
  {
    ...
    if (lev < finest_level)
    {
      ...
      LevelData<FArrayBox> &fineScal = *fineML.m_scalarNew[a_var];
...
Real AMRLevelMushyLayer::doHCreflux()
{
  ...
  for (int lev = startLev; lev <= finest_level; lev++)
  {
    AmrDx[lev] = thisMLPtr->m_dx;
    const DisjointBoxLayout &levelGrids = thisMLPtr->m_grids;
    // recall that AMRMultiGrid can only do one component. Bullshit! Can do more
    // now. rhs has no ghost cells. Bullshit!  ghost cell as AMRFASMultigrid
    // creates objects from this and they need a ghost cell
    HCRefluxRHS[lev] =
        new LevelData<FArrayBox>(levelGrids, numComps, IntVect::Unit);

    // soln has one layer of ghost cells
    IntVect ghostVect(D_DECL(1, 1, 1));
    HCRefluxCorr[lev] =
        new LevelData<FArrayBox>(levelGrids, numComps, ghostVect);
  ...
  for (int lev = m_level; lev <= finest_level; lev++)
  {
    ...
    // initial guess for correction is RHS
    HCRefluxRHS[lev]->copyTo(solverComps, *(HCRefluxCorr[lev]), solverComps);
  ...
  Real maxRefluxRHS =
      computeSum(HCRefluxRHS, refRat, m_dx, Interval(0, numComps - 1), m_level);

  int numLevels = finest_level + 1;

  // Need to define a two component VCAMRPoissonOp to solve for
  // enthalpy-salinity corrections
  ...
  Vector<RefCountedPtr<LevelData<FArrayBox>>> enthalpySolidus(finest_level + 1),
      enthalpyLiquidus(finest_level + 1), enthalpyEutectic(finest_level + 1),
      HC(finest_level + 1);
  ...
  for (int lev = baseLevel; lev <= finest_level; lev++)
  {
    ...
    HC[lev] = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(grids[lev], numComps, ivGhost));

    amrML->fillHC(*HC[lev], m_time);
    amrML->fillScalars(*enthalpySolidus[lev], m_time, m_enthalpySolidus, true,
                       true);
    amrML->fillScalars(*enthalpyLiquidus[lev], m_time, m_enthalpyLiquidus, true,
                       true);
    amrML->fillScalars(*enthalpyEutectic[lev], m_time, m_enthalpyEutectic, true,
                       true);
    ...
    for (DataIterator dit = bCoef[lev]->dataIterator(); dit.ok(); ++dit)
    {
      ...
      if (m_opt.refluxMethod != RefluxMethod::LinearReflux)
      {
        ...
        for (int dir = 0; dir < SpaceDim; dir++)
        {
          (*bCoef[lev])[dit][dir].mult(m_parameters.heatConductivityRatio,
                                       Hcomp);
        }
  ...
  if (m_opt.refluxMethod == RefluxMethod::LinearReflux ||
      m_opt.refluxMethod == RefluxMethod::LinearVCReflux)
  {

    VCAMRPoissonOp2Factory diffusiveOpFactory;
    diffusiveOpFactory.define(lev0Dom, grids, refRat, AmrDx[0], refluxBC, alpha,
                              aCoef, beta, bCoef);
    ...
    diffusionSolver->define(lev0Dom, castFact, &bottomSolver, numLevels);
  }
    ...
    for (int lev = m_level; lev <= finest_level; lev++)
    {
      ...
      for (DataIterator dit = HC[lev]->dataIterator(); dit.ok(); ++dit)
      {
        ...
        FORT_CALCULATE_BOUNDING_ENERGY(
            CHF_CONST_FRA(HCfab), CHF_FRA(Hs), CHF_FRA(He), CHF_FRA(Hl),
            CHF_BOX(region), CHF_CONST_REAL(m_parameters.compositionRatio),
...
#endif
            (CHF_CONST_FRA(HCfab), CHF_CONST_FRA(Hs), CHF_CONST_FRA(He),
             CHF_CONST_FRA(Hl),
#if CH_SPACEDIM >= 1
             CHF_FRA(thisBCoef[0]),
#endif
  ...
  {
    ops[ilev] = dynamic_cast<LevelTGAHelmOp<LevelData<FArrayBox>, FluxBox> *>(
        generic_ops[ilev]);
    if (ops[ilev] == nullptr)
    {
      MayDay::Error(
  ...
  // solve
  diffusionSolver->solve(HCRefluxCorr, HCRefluxRHS, finest_level, m_level,
                         false, // don't initialize to zero
  ...
  for (int lev = finest_level; lev >= m_level; lev--)
  {
    ...
    for (levelDit.reset(); levelDit.ok(); ++levelDit)
    {
      ...
      //(*thisMLPtr->m_scalarNew[ScalarVars::m_enthalpy])[levelDit()].plus(levelCorr[levelDit()],
      //0, 0, 1);
      //(*thisMLPtr->m_scalarNew[ScalarVars::m_bulkConcentration])[levelDit()].plus(levelCorr[levelDit()],
      //1, 0, 1);

      if (m_opt.reflux_enthalpy)
      {
        (*thisMLPtr->m_scalarNew[ScalarVars::m_enthalpy])[levelDit()].plus(
            levelCorr[levelDit()], m_opt.refluxCorrSign, 0, 0, 1);
      }

      if (m_opt.reflux_concentration)
      {
        (*thisMLPtr->m_scalarNew[ScalarVars::m_bulkConcentration])[levelDit()]
            .plus(levelCorr[levelDit()], m_opt.refluxCorrSign, 1, 0, 1);
      }
    }
