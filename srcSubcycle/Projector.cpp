...
// ------------------------------------------------------
int Projector::getLevel() const { return m_level; }

// ------------------------------------------------------
const ProblemDomain &Projector::dProblem() const { return m_domain; }

// ------------------------------------------------------
const DisjointBoxLayout &Projector::getBoxes() const { return m_Pi.getBoxes(); }

// ------------------------------------------------------
int Projector::nRefCrse() const { return m_nRefCrse; }

// ------------------------------------------------------
Real Projector::dx() const { return m_dx; }

// ------------------------------------------------------
Real Projector::etaLambda() const { return m_etaLambda; }

void Projector::etaLambda(Real a_eta) { m_etaLambda = a_eta; }

// ------------------------------------------------------
Projector *Projector::fineProjPtr() const { return m_fineProjPtr; }

// ------------------------------------------------------
Projector *Projector::crseProjPtr() const { return m_crseProjPtr; }

// ------------------------------------------------------
LevelData<FArrayBox> &Projector::phi() { return m_phi; }
...
// ------------------------------------------------------
const LevelData<FArrayBox> &Projector::phi() const { return m_phi; }

// ------------------------------------------------------
LevelData<FArrayBox> &Projector::eSync() { return m_eSync; }

// ------------------------------------------------------
const LevelData<FArrayBox> &Projector::eSync() const { return m_eSync; }

// ------------------------------------------------------
LevelData<FArrayBox> &Projector::MACrhs() { return m_MACrhs; }

// ------------------------------------------------------
LevelData<FArrayBox> &Projector::CCrhs() { return m_CCrhs; }

LevelData<FluxBox> &Projector::MACcorrection() { return m_MACcorrection; }

LevelData<FArrayBox> &Projector::CCcorrection() { return m_CCcorrection; }

LevelData<FArrayBox> &Projector::Pi() { return m_Pi; }
...
// ------------------------------------------------------
LevelData<FArrayBox> &Projector::eLambda() { return m_eLambda; }

// ------------------------------------------------------
const LevelData<FArrayBox> &Projector::eLambda() const { return m_eLambda; }

// ------------------------------------------------------
LevelData<FluxBox> &Projector::grad_eLambda() { return m_grad_eLambda; }
...
// ------------------------------------------------------
bool Projector::doSyncProjection() const { return m_doSyncProjection; }

// ------------------------------------------------------
bool Projector::doMacSync() const { return (m_etaLambda != 0.0); }

// ------------------------------------------------------
bool Projector::isInitialized() const { return m_isInitialized; }

// ------------------------------------------------------
bool Projector::doQuadInterp() const { return m_doQuadInterp; }

// ------------------------------------------------------
QuadCFInterp &Projector::quadCFInterpolator() { return m_cfInterp; }

/// returns predefined intvectset which shows coverage
const LayoutData<IntVectSet> &Projector::getGridIVS() { return m_gradIVS; }

// ------------------------------------------------------
bool Projector::isFinestLevel() const { return m_finest_level; }
...
// ------------------------------------------------------
void Projector::verbosity(int a_verbosity) { s_verbosity = a_verbosity; }
//
...
// ------------------------------------------------------
int Projector::verbosity() const { return s_verbosity; }
...
// ------------------------------------------------------
PhysBCUtil *Projector::getPhysBCPtr() const { return m_physBCPtr; }

// ------------------------------------------------------
// default constructor
Projector::Projector()
    : m_sumVDrhs(0.0), m_usePiAdvectionBCs(true),
      m_scalePressureWithPorosity(true), m_scaleSyncCorrection(true),
      m_level(-1), m_dx(-1), m_fineProjPtr(nullptr), m_crseProjPtr(nullptr),
      m_nRefCrse(-1), m_isInitialized(false), m_limitSolverCoarsening(false),
      m_finest_level(false), m_phiScale(1.0), m_MACbcScale(-0.5),
      m_scale_lambda_with_porosity(false),
      m_scale_lambda_err_with_porosity(false), m_bottomSolverLevel(nullptr),
      m_BiCGBottomSolverLevel(nullptr), m_physBCPtr(nullptr) {}
...
Projector::Projector(const DisjointBoxLayout &a_grids,
                     ...
                     const Box &a_domain, const Real a_dx,
                     Projector *a_finerProjPtr, Projector *a_crseProjPtr,
...
{
  m_physBCPtr = nullptr;
  define(a_grids, a_crseGridsPtr, a_domain, a_dx, a_finerProjPtr, a_crseProjPtr,
         a_nRefCrse, a_level, a_physBC);
}
                       ...
                       bool a_usePiAdvectionBCs)
{
  ...
  if (m_crseProjPtr != nullptr)
  {
    ...
    CH_assert(a_crseGridsPtr != nullptr);

    m_cfInterp.define(a_grids, a_crseGridsPtr, a_dx, a_nRefCrse, 1, a_domain);
  }
  ...
  m_Pi.define(a_grids, 1,
              2 * ghostVect); // need two ghost cells here for CC projection
...
void Projector::variableSetUp()
{
  ...
  ppProjection.query("scale_lambda_with_porosity",
                     m_scale_lambda_with_porosity);
...
void Projector::setCrseProj(Projector *a_crseProjPtr, int a_nRefCrse)
{
  if (m_crseProjPtr != nullptr)
  {
    ...
    CH_assert(a_crseProjPtr != nullptr);
...
void Projector::writeCheckpointLevel(HDF5Handle &a_handle) const
{
  ...
  const std::string label =
      std::string("level_") + level_str + std::string("/projection");
...
void Projector::readCheckpointLevel(HDF5Handle &a_handle)
{
  ...
  {
    MayDay::Error("CCProjector::readCheckpointLevel: dx in checkfile does not "
                  "match that in solver");
  }
  ...
  {
    MayDay::Error(
        "CCProjector::readCheckpointLevel: file does not contain domain");
    if (m_domain.domainBox() != header.m_box["domain"])
    {
      MayDay::Error("CCProjector::readCheckpointLevel: domain in checkfile "
                    "does not match that in solver!");
    }
  }
  ...
  {
    MayDay::Error(
        "CCProjector::readCheckpointLevel: file does not contain n_ref_crse");
    if (m_nRefCrse != header.m_int["n_ref_crse"])
    {
      MayDay::Error("CCProjector::readCheckpointLevel: n_ref_crse in checkfile "
                    "does not match that in solver!");
    }
  }
  ...
  {
    MayDay::Error(
        "CCProjector::readCheckpointLevel: file does not contain finest_level");
  }
  ...
  if (grid_status != 0)
  {
    MayDay::Error("CCProjector::readCheckpointLevel: file does not contain a "
                  "Vector<Box>");
  }
  // can't really check for equality, but at least check for same size...
  if (grids.size() != m_Pi.getBoxes().size())
  {
    MayDay::Error("CCProjector::readCheckpointLevel: grids in checkfile "
                  "different size than in solver!");
  }
  ...
  {
    MayDay::Error(
        "CCProjector::readCheckpointLevel: file does not contain pi data");
  }

  const int phiData_status =
      read<FArrayBox>(a_handle, m_phi, "phi", levelGrids);

  if (phiData_status != 0)
  {
    MayDay::Error(
        "CCProjector::readCheckpointLevel: file does not contain phi data");
  }

  const int eSyncData_status =
      read<FArrayBox>(a_handle, m_eSync, "e_sync", levelGrids);

  if (eSyncData_status != 0)
  {
    MayDay::Error(
        "CCProjector::readCheckpointLevel: file does not contain e_sync data");
  }

  const int eLambdaData_status =
      read<FArrayBox>(a_handle, m_eLambda, "e_lambda", levelGrids);

  if (eLambdaData_status != 0)
  {
    MayDay::Error(
        "CCProjector::readCheckpointLevel: file does not contain eLambda data");
  }

  const int grad_eLambdaData_status =
      read<FluxBox>(a_handle, m_grad_eLambda, "grad_e_lambda", levelGrids);

  if (grad_eLambdaData_status != 0)
  {
    MayDay::Error("CCProjector::readCheckpointLevel: file does not contain "
                  "grad_e_lambda data");
  }
...
void Projector::gradPhi(LevelData<FArrayBox> &a_gradPhi, int a_dir) const
{
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    ...
    Gradient::singleBoxMacGrad(a_gradPhi[dit], m_phi[dit], 0, 0, 1, grids[dit],
                               m_dx, a_dir, edgeDir, m_gradIVS[dit]);
  } // end loop over grids
}
...
void Projector::gradPi(LevelData<FArrayBox> &a_gradPi, int a_dir) const
{
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    ...
    FORT_GRADCC(CHF_FRA1(a_gradPi[dit], 0), CHF_CONST_FRA1(m_Pi[dit], 0),
                CHF_BOX(getBoxes()[dit]), CHF_CONST_REAL(m_dx), CHF_INT(a_dir));
  }
}
...
void Projector::gradPiBCs(LevelData<FArrayBox> &a_gradPi, bool extrapBCs,
                          bool a_usePhi)
{
  ...
  LevelData<FArrayBox> pressureTemp(m_Pi.disjointBoxLayout(), 1,
                                    m_Pi.ghostVect());
  if (a_usePhi)
  {
    for (dit.reset(); dit.ok(); ++dit)
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    ...
    bcHolder.operator()(pressureTemp[dit], b, m_domain, m_dx,
...
void Projector::grad_eSync(LevelData<FArrayBox> &a_grad_eSync) const
{
  if (m_fineProjPtr != nullptr)
  {
    ...
    Gradient::compGradientCC(a_grad_eSync, m_eSync, &fine_eSync, m_dx, nRefFine,
                             m_domain);
  }
  else
  {
...
// note that this function assumes all BC's on eSync have been set
// also only returns valid grad(eSync) in valid regions of grids
void Projector::grad_eSync(LevelData<FArrayBox> &a_grad_eSync,
                           int a_dir) const
{
...
int Projector::levelMacProject(
    ...
    bool alreadyHasPhi, Real correctScale)
{
  if (s_verbosity >= 5)
  {
  ...
  if (s_verbosity >= 2)
  {
    ...
    int nRefFine = -1;
    Real sumRHS = computeSum(MACrhs(), finerGridsPtr, nRefFine, m_dx,
                             MACrhs().interval());
    ...
    Real sumRHSInterior = ::computeSum(rhsInterior, finerGridsPtr, nRefFine,
                                       m_dx, MACrhs().interval());
    LOG("  MAC projection (level "
        << m_level << ") -- sum(interior RHS) = " << sumRHSInterior);
  }

  DataIterator dit = m_phi.dataIterator();

  LevelData<FArrayBox> oldPhi(m_phi.disjointBoxLayout(), m_phi.nComp(),
                              m_phi.ghostVect());
  ...
  int exitStatus =
      solveMGlevel(m_phi, a_crsePressurePtr, MACrhs(), a_pressureScalePtr,
                   a_crsePressureScalePtr, a_pressureScaleEdgePtr,
                   a_crsePressureScaleEdgePtr);
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    bcHolder.operator()(m_phi[dit], levelGrids[dit], m_domain, m_dx,
                        false); // not homogeneous
  }

  //#define CORNERCOPIER
  //#ifdef CORNERCOPIER
  CornerCopier cornerExchangeCopier(m_phi.getBoxes(), m_phi.getBoxes(),
                                    m_domain, m_phi.ghostVect(), true);
  ...
  // compute gradient
  Gradient::levelGradientMAC(gradPhi, m_phi, a_crsePressurePtr, m_dx, m_gradIVS,
                             m_cfInterp);
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    ...
    for (int dir = 0; dir < SpaceDim; dir++)
    {
      ...
      if (m_porosityEdgePtr != nullptr)
      {
        thisGradDir.mult((*m_porosityEdgePtr)[dit][dir], thisGradDir.box(), 0,
                         0);
      }
...
void Projector::checkDivergence(LevelData<FluxBox> &a_uEdge)
{
  ...
  Real maxDivU = ::computeNorm(DivU, nullptr, 1, m_dx, Interval(0, 0), 0);
  ...
  if (s_verbosity >= 2)
  {
    LOG("  Projector::checkDivergence - Setting solver tolerance = "
        << s_solver_tolerance);
  }
}

Real Projector::getPhiScale(Real a_dt) { return getScale(m_phiScale, a_dt); }
...
// --------------------------------------------------------------
int Projector::levelMacProject(
    LevelData<FluxBox> &a_uEdge, Real a_oldTime, Real a_dt,
    ...
    const RefCountedPtr<LevelData<FluxBox>> a_crsePorosityEdgePtr)
{
  ...
  LevelData<FArrayBox> *pressureBCPtr = nullptr;
  ...
  if (s_verbosity >= 2)
  {
    ...
    Real sumRHS = computeSum(MACrhs(), finerGridsPtr, nRefFine, m_dx,
                             MACrhs().interval());
    LOG("    MAC projection (level " << m_level
                                     << ") -- sum(RHS) = " << sumRHS);
  }
  ...
  // now solve for phi
  int exitStatus =
      solveMGlevel(m_phi, pressureBCPtr, MACrhs(), a_porosityPtr,
                   a_crsePorosityPtr, a_porosityEdgePtr, a_crsePorosityEdgePtr);
...
#define CORNERCOPIER
  ...
  if (pressureBCPtr != nullptr && phiScale != a_dt)
  {
    delete pressureBCPtr;
    pressureBCPtr = nullptr;
  }
  // that should be it!
    ...
    if (maxDivU > 1e-10 && maxVel / maxDivU > 100)
    {
      ...
      for (dit.reset(); dit.ok(); ++dit)
      {
        ...
        for (int dir = 0; dir < SpaceDim; dir++)
        {
          ...
          //        if (m_porosityEdgePtr != nullptr)
          //        {
          //          thisGradDir.mult((*m_porosityEdgePtr)[dit][dir],thisGradDir.box(),
          //          0, 0);
          //        }
                                   ...
                                   Real phiScale)
{
  ...
  // compute gradient
  Gradient::levelGradientMAC(gradPhi, m_phi, crseBCDataPtr, m_dx, m_gradIVS,
                             m_cfInterp);
...
void Projector::setPressureScalePtr(
    RefCountedPtr<LevelData<FArrayBox>> a_pressureScalePtr)
{
  m_porosityPtr = a_pressureScalePtr;
}

void Projector::setPressureScaleEdgePtr(
    RefCountedPtr<LevelData<FluxBox>> a_pressureScaleEdgePtr)
{
  m_porosityEdgePtr = a_pressureScaleEdgePtr;
}

void Projector::getCFBC(LevelData<FArrayBox> &velBC,
                        ...
                        LevelData<FArrayBox> *a_crsePorosityPtr, Real a_dt)
{
  ...
  if (doQuadInterp())
  {
    ...
    //              velBCPtr = new LevelData<FArrayBox>(crseLevelGrids,
    //              SpaceDim); LevelData<FArrayBox>& crseData = *velBCPtr;
    ...
    for (ditCrse.reset(); ditCrse.ok(); ++ditCrse)
    {
      ...
      else
      {
        ...
        // we need to use dt on the crse level so we remove the entire grad(P)
        // from the crse vel
        thisCrseGradPi *= 0;
      }

      if (a_crsePorosityPtr != nullptr)
      {
    ...
    const bool a_isViscous)
{
  ...
  LevelData<FArrayBox> *velBCPtr = nullptr;
  ...
  if (m_level > 0)
  {
    ...
    if (doQuadInterp() && m_crseProjPtr != nullptr)
    {
      ...
      velCFInterp.define(a_velocity.getBoxes(), &(velBCPtr->getBoxes()), m_dx,
                         m_nRefCrse, SpaceDim, m_domain);
    }
  }
  ...
  Divergence::levelDivergenceCC(CCrhs(), a_velocity, velBCPtr, m_dx,
                                doQuadInterp(), velCFInterp);
  ...
  if (s_verbosity >= 2)
  {
    ...
    int nRefFine = -1;
    Real sumRHS =
        computeSum(CCrhs(), finerGridsPtr, nRefFine, m_dx, CCrhs().interval());
  ...
  // now solve for pi
  solveMGlevel(m_Pi, crsePiPtr, CCrhs(), a_porosityPtr, a_crsePorosityPtr,
               a_porosityEdgePtr, a_crsePorosityEdgePtr,
               true); // cell centred

  // apply appropriate physical BC's
  BCHolder bcHolder =
      m_physBCPtr->gradPiFuncBC(); // this is what CC project used to use
  //  BCHolder bcHolder = m_physBCPtr->BasicPressureFuncBC(false); // this is
  //  what MAC project uses
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    bcHolder.operator()(m_Pi[dit], levelGrids[dit], m_domain, m_dx,
                        false); // not homogeneous
  }
  ...
  if (abs(alpha) > 0.0)
  {
    ...
    Divergence::levelDivergenceCC(divU, a_velocity, velBCPtr, m_dx,
                                  doQuadInterp(), velCFInterp);
                                       ...
                                       const bool a_isViscous)
{
  ...
  LevelData<FArrayBox> pressure(a_velocity.disjointBoxLayout(), 1,
                                IntVect::Unit);
  ...
  // now compute RHS for projection
  // BCs should already be set
  Divergence::levelDivergenceCC(rhs, a_velocity, velBCPtr, m_dx, doQuadInterp(),
                                velCFInterp);
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    bcHolder.operator()(pressure[dit], levelGrids[dit], m_domain, m_dx,
                        false); // not homogeneous
  }
  ...
  // dt scale was 1/dt, so correct scale is dt
  Real correctScale = 1 / dtScale; // 1/dtScale;
  applyCCcorrection(a_velocity, &pressure, nullptr, correctScale);
}
...
// ---------------------------------------------------------------
void Projector::doSyncOperations(
    ...
    const Real a_newTime, const Real a_dtLevel)
{
  ...
  AMRMultiGrid<LevelData<FArrayBox>> *dspSolver =
...
// ---------------------------------------------------------------
void Projector::doPostRegridOps(
    Vector<LevelData<FArrayBox> *> &a_lambda,
    Vector<RefCountedPtr<LevelData<FluxBox>>> &a_porosity, const Real a_dt,
    const Real a_time, const Real a_etaScale)
{
  AMRMultiGrid<LevelData<FArrayBox>> *bigSolverPtr =
      new AMRMultiGrid<LevelData<FArrayBox>>;

  defineMultiGrid(*bigSolverPtr, a_lambda, a_porosity,
                  true); // true - freestream solve
...
// ---------------------------------------------------------------
void Projector::doSyncProjection(
    Vector<LevelData<FArrayBox> *> &a_velocity,
    Vector<RefCountedPtr<LevelData<FArrayBox>>> &a_porosity,
    ...
    AMRMultiGrid<LevelData<FArrayBox>> &a_solver)
{
  if (doSyncProjection())
  {
    ...
    for (int lev = m_level; lev <= finestLevel; lev++)
    {
      ...
      Divergence::compDivergenceCC(*syncRHS[lev], *a_velocity[lev], crseVelPtr,
    ...
    LOG_INFO("  Sum(RHS) for sync solve = " << setiosflags(ios::scientific)
                                            << sumRHS);
    ...
    for (int lev = m_level; lev <= finestLevel; lev++)
    {
      ...
      {
        bcHolder.operator()(level_eSync[ditLev], levelGrids[ditLev],
                            levelDomain, levelDx,
                            false); // not homogeneous
      }
    ...
    if (m_level > 0)
      crseCorrPtr = syncCorr[m_level - 1];
...
// ---------------------------------------------------------------
void Projector::applySyncCorrection(
    Vector<LevelData<FArrayBox> *> &a_velocities,
    Vector<RefCountedPtr<LevelData<FArrayBox>>> &a_porosity, const Real a_scale,
    LevelData<FArrayBox> *crseCorrPtr)
{
  ...
  if (m_applySyncCorrection)
  {
    ...
    Gradient::compGradientCC(levelGradCorr, m_eSync, crseCorrPtr, fineCorrPtr,
                             m_dx, nRefFine, m_domain, m_gradIVS, m_cfInterp);
...
void Projector::computeVDCorrection(
    Vector<LevelData<FArrayBox> *> &a_lambda,
    Vector<RefCountedPtr<LevelData<FluxBox>>> &a_porosity, const Real a_newTime,
    const Real a_dtSync)
{

  AMRMultiGrid<LevelData<FArrayBox>> *cvdcSolver =
      new AMRMultiGrid<LevelData<FArrayBox>>;
  defineMultiGrid(*cvdcSolver, a_lambda, a_porosity, true);
...
void Projector::computeVDCorrection(
    Vector<LevelData<FArrayBox> *> &a_lambda,
    Vector<RefCountedPtr<LevelData<FluxBox>>> &a_porosity, const Real a_newTime,
    const Real a_dtSync, AMRMultiGrid<LevelData<FArrayBox>> &a_solver)
{
  if (s_verbosity > 3)
  {
  ...
  if (doMacSync())
  {
    ...
    // We only store the ref ratio to the coarser level, so have to do this in a
    // slightly backwards way
    for (int lev = m_level; lev <= finestLevel; lev++)
    {
      if (lev > 0)
      {
    ...
    //    Real maxLambda = ::computeNorm(a_lambda, nRefFineVect, m_dx,
    //    Interval(0,0), 0, m_level);
    ...
    for (int lev = m_level; lev < vectorSize; lev++)
    {
      ...
      newLambda[lev] = new LevelData<FArrayBox>(
          a_lambda[lev]->disjointBoxLayout(), 1, a_lambda[lev]->ghostVect());
    ...
    Real maxLambda =
        ::computeNorm(a_lambda, nRefFineVect, m_dx, sumComps, m_level);
    m_sumVDrhs = m_sumVDrhs / m_etaLambda;
    LOG_INFO("  Sum(RHS) for VD solve on level "
             << m_level << " (divided by eta) = "
             << setiosflags(ios::scientific) << m_sumVDrhs << " (current time "
             << a_newTime << "), max|lambda| = " << maxLambda);
    ...
    // Unsure why we were using extrap BCs here before. Using the freestream BCs
    // instead. If we don't use freestream BCs, we don't conserve fluxes with
    // neumann BCs...
    ...
    for (int lev = m_level; lev <= finestLevel; lev++)
    {
      ...
      for (ditLev.reset(); ditLev.ok(); ++ditLev)
      {
        bcHolder.operator()(levelCorr[ditLev], levelGrids[ditLev], levelDomain,
                            levelDx,
                            false); // not homogeneous
      }
    ...
    for (int lev = m_level; lev < vectorSize; lev++)
    {
      if (newLambda[lev] != nullptr)
      {
        delete newLambda[lev];
        newLambda[lev] = nullptr;
      }
    }
...
// ---------------------------------------------------------------
void Projector::computeGrad_eLambda(
    Vector<RefCountedPtr<LevelData<FluxBox>>> &a_porosity)
{
  ...
  LevelData<FluxBox> temp_grad_eLambda(m_grad_eLambda.disjointBoxLayout(),
                                       m_grad_eLambda.nComp(),
                                       m_grad_eLambda.ghostVect());
  for (DataIterator dit = temp_grad_eLambda.dataIterator(); dit.ok(); ++dit)
  {
    temp_grad_eLambda[dit].setVal(0.0);
  }

  Gradient::levelGradientMAC(temp_grad_eLambda, m_eLambda, crseBCPtr, m_dx,
                             m_gradIVS, m_cfInterp);
  ...
  for (DataIterator dit = temp_grad_eLambda.dataIterator(); dit.ok(); ++dit)
  {
    ...
    if (m_scaleSyncCorrection ||
        m_scale_lambda_err_with_porosity) // || scale_lambda_err_with_porosity ?
    {
    ...
    pp.query("divide_lambda_correction_porosity",
             divide_lambda_correction_porosity);
...
void Projector::applyFreestreamCorrection(LevelData<FluxBox> &a_advVel,
                                          Real scale)
{
...
// ---------------------------------------------------------------
void Projector::initialLevelProject(
    LevelData<FArrayBox> &a_velocity, LevelData<FArrayBox> *a_crseVelPtr,
    const Real a_newTime, const Real a_dt,
    const RefCountedPtr<LevelData<FArrayBox>> a_porosityPtr,
    const RefCountedPtr<LevelData<FArrayBox>> a_crsePorosityPtr,
    ...
    const RefCountedPtr<LevelData<FluxBox>> a_crsePorosityEdgePtr)
{
    ...
    if (doQuadInterp())
    {
      ...
      for (ditCrse.reset(); ditCrse.ok(); ++ditCrse)
      {
        ...
        if (a_crsePorosityPtr != nullptr)
        {
          for (int dir = 0; dir < SpaceDim; dir++)
          {
            thisCrseGradPi.mult((*a_crsePorosityPtr)[ditCrse],
                                thisCrseGradPi.box(), 0, dir);
          }
        }
  ...
  // now compute RHS for projection
  Divergence::levelDivergenceCC(levelProjRHS, a_velocity, crseDataPtr, m_dx,
                                doQuadInterp(), m_cfInterp);
...
// ---------------------------------------------------------------
void Projector::initialSyncProjection(
...
// ---------------------------------------------------------------
void Projector::doInitialSyncOperations(
    Vector<LevelData<FArrayBox> *> &a_vel,
    Vector<LevelData<FArrayBox> *> &a_lambda,
    Vector<RefCountedPtr<LevelData<FluxBox>>> &a_porosityFace,
    Vector<RefCountedPtr<LevelData<FArrayBox>>> &a_porosity,
    const Real a_newTime, const Real a_dtSync)
{
  ...
  AMRMultiGrid<LevelData<FArrayBox>> *ispSolverPtr =
      new AMRMultiGrid<LevelData<FArrayBox>>;
  ...
  delete ispSolverPtr;
  AMRMultiGrid<LevelData<FArrayBox>> *cvdcSolverPtr =
      new AMRMultiGrid<LevelData<FArrayBox>>;
  defineMultiGrid(*cvdcSolverPtr, a_lambda, a_porosityFace, true);

  // now do freestream preservation solve
  computeVDCorrection(a_lambda, a_porosityFace, a_newTime, a_dtSync,
                      *cvdcSolverPtr);
...
// ---------------------------------------------------------------
void Projector::initialVelocityProject(
    Vector<LevelData<FArrayBox> *> &a_vel,
    Vector<RefCountedPtr<LevelData<FluxBox>>> &a_porosityFace,
    Vector<RefCountedPtr<LevelData<FArrayBox>>> &a_porosity,
...
void Projector::initialVelocityProject(
    Vector<LevelData<FArrayBox> *> &a_vel,
    Vector<RefCountedPtr<LevelData<FArrayBox>>> &a_porosity,
    AMRMultiGrid<LevelData<FArrayBox>> &a_bigSolver, bool a_homogeneousCFBC)
{
  // set up temp storage
  ...
  for (int lev = m_level; lev <= finestLevel; lev++)
  {
    ...
    // now compute divergence
    //    fineVelPtr = nullptr; // this was just testing
  ...
  LOG_INFO("Sum(RHS) for IVP solve = " << setiosflags(ios::scientific)
                                       << sumRHS);
...
// ---------------------------------------------------------------
void Projector::defineMultiGrid(
    AMRMultiGrid<LevelData<FArrayBox>> &a_solver,
    ...
    bool a_freestreamSolve)
{
  ...
  // Since we are always going to need the grids and coefficients on this level,
  // do that now
  allGrids[m_level] = a_vel[m_level]->getBoxes();
  aCoef[m_level] = RefCountedPtr<LevelData<FArrayBox>>(
      new LevelData<FArrayBox>(allGrids[m_level], 1));
  setValLevel(*aCoef[m_level], 1.0);

  // Now deal with finer levels
  for (int lev = m_level + 1; lev <= finestLevel; lev++)
  {
    levelProj = levelProj->fineProjPtr(); // levelProj now points to the same
                                          // level as index lev
    allGrids[lev] = a_vel[lev]->getBoxes();
    allRefRatios[lev - 1] =
        levelProj->nRefCrse(); // ref ratio to next finer level belongs to that
                               // level for CCProjector

    aCoef[lev] = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(allGrids[lev], 1));
  ...
  for (int lev = m_level - 1; lev >= 0; lev--)
  {
    ...
    allRefRatios[lev] =
        levelProj->nRefCrse(); // levelProj points to the level above lev

    levelProj =
        levelProj
            ->crseProjPtr(); // lecvelProj now points to the same level as lev
  ...
  // levelProj is now at level 0. This is crucial for making the LHS operator
  // consistent
  ...
  localPoissonOpFactory.define(baseDomain, allGrids, allRefRatios, baseDx,
                               ((a_freestreamSolve)
  ...
  opFact.define(baseDomain, allGrids, allRefRatios, baseDx,
                ((a_freestreamSolve) ? m_physBCPtr->FreestreamCorrFuncBC()
                                     : m_physBCPtr->SyncProjFuncBC()),
  ...
  if (m_scaleSyncCorrection)
  {
    a_solver.define(baseDomain, opFact, m_bottomSolverLevel, finestLevel + 1);
  }
  else
  {
    a_solver.define(baseDomain, localPoissonOpFactory, m_bottomSolverLevel,
                    finestLevel + 1);
  }
    ...
    bool cellCentred, Real beta)
{
  ...
  if (a_crsePorosityEdgePtr != nullptr)
  {
    numSolverLevels = 2;
  }
  ...
  if (a_crsePorosityEdgePtr != nullptr)
  {
    m_aCoef[0] = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(m_allGrids[0], 1, IntVect::Unit));
    m_aCoef[1] = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(m_allGrids[1], 1, IntVect::Unit));
    //    m_bCoefCC[1] = RefCountedPtr<LevelData<FArrayBox> >(new
    //    LevelData<FArrayBox>(m_allGrids[1], 1, 2*IntVect::Unit));
    setValLevel(*m_aCoef[0], 1.0);
    setValLevel(*m_aCoef[1], 1.0);

    //    m_bCoef[0] = a_crsePorosityEdgePtr;
    //    m_bCoef[1] = a_porosityEdgePtr;
    m_bCoef[0] = RefCountedPtr<LevelData<FluxBox>>(
        new LevelData<FluxBox>(m_allGrids[0], 1, IntVect::Unit));
    m_bCoef[1] = RefCountedPtr<LevelData<FluxBox>>(
        new LevelData<FluxBox>(m_allGrids[1], 1, IntVect::Unit));
    m_bCoefCC[0] = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(m_allGrids[0], 1, IntVect::Unit));
    m_bCoefCC[1] = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(m_allGrids[1], 1, IntVect::Unit));
  ...
  faceOpFact.define(baseDomain, m_allGrids, refRatios, dxCrse,
                    m_physBCPtr->LevelPressureFuncBC(), alpha, m_aCoef, beta,
                    m_bCoef, average_type);
  ...
  if (s_relax_bottom_solver)
  {
    m_solverMGlevel.define(baseDomain, // on either this level or coarser level
                           faceOpFact, m_bottomSolverLevel, numSolverLevels);
  }
  else
  {
    m_solverMGlevel.define(baseDomain, // on either this level or coarser level
                           faceOpFact, m_BiCGBottomSolverLevel,
                           numSolverLevels);
  }
...
void Projector::makeBottomSolvers()
{
  ...
  if (m_BiCGBottomSolverLevel != nullptr)
  {
  ...
  BiCGStabSolver<LevelData<FArrayBox>> *newBottomPtr =
      new BiCGStabSolver<LevelData<FArrayBox>>;
  RelaxSolver<LevelData<FArrayBox>> *newRelaxBottomPtr =
      new RelaxSolver<LevelData<FArrayBox>>;
...
void Projector::setSolverParameters()
{
  ...
  m_solverMGlevel.m_bottom =
      s_num_precond_smooth; // smoothing before bottom solve
                                    ...
                                    const DisjointBoxLayout *a_crseGridsPtr)
{
  ...
  { // coarser level exists:  define solver on two levels
    ...
    localPoissonOpFactory.define(baseDomain, m_allGrids, refRatios, dxCrse,
  ...
  { // no coarser level:  define solver on only one level
    ...
    localPoissonOpFactory.define(m_domain, a_grids, m_dx,
                                 m_physBCPtr->LevelPressureFuncBC());
    //                                 alpha, beta);
  }
  ...
  {
    m_solverMGlevel.define(baseDomain, // on either this level or coarser level
                           localPoissonOpFactory, m_bottomSolverLevel,
                           numSolverLevels);
  }
  else
  {
    m_solverMGlevel.define(baseDomain, // on either this level or coarser level
                           localPoissonOpFactory, m_BiCGBottomSolverLevel,
                           numSolverLevels);
  }
...
{
  RefCountedPtr<LevelData<FArrayBox>> a_pressureScalePtr,
      a_crsePressureScalePtr;
  RefCountedPtr<LevelData<FluxBox>> a_pressureScaleEdgePtr,
      a_crsePressureScaleEdgePtr;

  solveMGlevel(a_phi, a_phiCoarsePtr, a_rhs, a_pressureScalePtr,
               a_crsePressureScalePtr, a_pressureScaleEdgePtr,
               a_crsePressureScaleEdgePtr, cellCentred);
}

// -------------------------------------------------------------
int Projector::solveMGlevel(
    ...
    bool cellCentred)
{
  ...
  LevelData<FArrayBox> &rhsRef = const_cast<LevelData<FArrayBox> &>(a_rhs);
  ...
  if (a_pressureScaleEdgePtr != nullptr)
  {
