...
void getAMRHierarchy(string inFile, Vector<AMRLevelMushyLayer *> &amrlevels,
                     int &finest_level, HDF5HeaderData &header)
{
  ...
  D_TERM(
      if (!(lev0header.m_int.find("is_periodic_0") == lev0header.m_int.end()))
  ...
  for (int level = 0; level <= finest_level; ++level)
  {
    ...
    amrlevels[level] =
        (static_cast<AMRLevelMushyLayer *>(mlFact->new_amrlevel()));
