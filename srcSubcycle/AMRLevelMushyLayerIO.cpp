...
void AMRLevelMushyLayer::writeAMRHierarchy(string filename)
{
  ...
  int numComps =
      SpaceDim * m_outputVectorVars.size() + m_outputScalarVars.size();

  Vector<AMRLevel *> hierarchy = AMRLevel::getAMRLevelHierarchy();
  int nlevels = hierarchy.size();

  Vector<AMRLevelMushyLayer *>
      a_hierarchy; // don't actually need this, except to fill the arguments in
  ...
  for (int lev = 0; lev < nlevels; lev++)
  {
    ...
    for (int vari = 0; vari < m_outputScalarVars.size(); vari++)
    {
      ...
      (*ml->m_scalarNew[thisVar])
          .copyTo(Interval(0, 0), *a_vectData[lev],
                  Interval(startComp, startComp));
    ...
    for (int vari = 0; vari < m_outputVectorVars.size(); vari++)
    {
      ...
      (*ml->m_vectorNew[thisVar])
          .copyTo(Interval(0, SpaceDim - 1), *a_vectData[lev],
                  Interval(startComp, startComp + SpaceDim - 1));
  ...
  for (int vari = 0; vari < m_outputVectorVars.size(); vari++)
  {
    for (int i = 0; i < SpaceDim; i++)
    {
      ...
      sprintf(vectName, "%s%s", component.c_str(),
              m_vectorVarNames[m_outputVectorVars[vari]].c_str());
      a_vectNames[m_outputScalarVars.size() + vari * SpaceDim + i] =
          string(vectName);
    }
  }

  getHierarchyAndGrids(a_hierarchy, a_vectGrids, a_vectRatio, a_baseDomain,
                       a_baseDx);
  ...
  WritePartialAMRHierarchyHDF5( // file to send output to
      ...
      // time
      a_time,
      // refinement ratios between adjacent levels, starting with
      // <i>a_refRatio</i>[0], the refinement ratio between levels 0 and 1;
      // Vector length at least <i>a_numLevels</i>-1
      a_vectRatio,
...
void AMRLevelMushyLayer::writeCheckpointHeader(HDF5Handle &a_handle) const
{
  LOG_FUNCTION_ENTRY();
...
/*******/
void AMRLevelMushyLayer::writeCheckpointLevel(HDF5Handle &a_handle) const
{
  LOG_FUNCTION_ENTRY();
  ...
  D_TERM(if (m_problem_domain.isPeriodic(0)) header.m_int["is_periodic_0"] = 1;
         else header.m_int["is_periodic_0"] = 0;
         ,

         if (m_problem_domain.isPeriodic(1)) header.m_int["is_periodic_1"] = 1;
         else header.m_int["is_periodic_1"] = 0;
         ,

         if (m_problem_domain.isPeriodic(2)) header.m_int["is_periodic_2"] = 1;
...
/*******/
void AMRLevelMushyLayer::readCheckpointHeader(HDF5Handle &a_handle)
{
  LOG_FUNCTION_ENTRY();
...
/*******/
void AMRLevelMushyLayer::readCheckpointLevel(HDF5Handle &a_handle)
{
  LOG_FUNCTION_ENTRY();
  // Setup the level string
  char levelStr[20];
  ...
  {
    MayDay::Error("AMRLevelMushyLayer::readCheckpointLevel: file does not "
                  "contain ref_ratio");
  }
  ...
  {
    MayDay::Error(
        "AMRLevelMushyLayer::readCheckpointLevel: file does not contain dx");
  }
  ...
  {
    MayDay::Error(
        "AMRLevelMushyLayer::readCheckpointLevel: file does not contain dt");
  }
  ...
  {
    MayDay::Error(
        "AMRLevelMushyLayer::readCheckpointLevel: file does not contain time");
  }
  ...
  for (int var = 0; var < m_numScalarVars; var++)
  {
    ...
    if (std::find(v.begin(), v.end(), var) != v.end())
    {
      ...
      dataStatus =
          read<FArrayBox>(a_handle, tempScalar, m_scalarVarNames[var], m_grids);
      ...
      if (dataStatus != 0)
      {
        if (s_verbosity >= 10)
        {
          LOG_INFO("       ... variable not found in checkpoint file, "
                   "continuing anyway ");
        }
        // This is no longer an error - just read in what we can and deal with
        // it
        //      MayDay::Error("AMRLevelMushyLayer::readCheckpointLevel: file
        //      does not contain state data");
      }
  ...
  for (int var = 0; var < m_numVectorVars; var++)
  {
    ...
    {
      dataStatus =
          read<FArrayBox>(a_handle, tempVector, m_vectorVarNames[var], m_grids);
      tempVector.copyTo(Interval(0, SpaceDim - 1), *m_vectorNew[var],
                        Interval(0, SpaceDim - 1));
      //      if (dataStatus != 0)
      //      {
      // LOG_INFO("       ... variable not found in checkpoint file, continuing
      // anyway ");
      //     }
    }
    else
  ...
  if (m_opt.refTemp != m_opt.prevRefTemp &&
      m_opt.refSalinity != m_opt.prevRefSalinity)
  {
  ...
  LOG_INFO("dt reduced by factor " << m_opt.dtReductionFactor << " to "
                                   << m_dt);
...
/*******/
void AMRLevelMushyLayer::writePlotHeader(HDF5Handle &a_handle) const
{
  LOG_FUNCTION_ENTRY();
...
/*******/
void AMRLevelMushyLayer::writePlotLevel(HDF5Handle &a_handle) const
{
  LOG_FUNCTION_ENTRY();
  ...
  if (s_verbosity >= 5)
  {
    LOG_INFO("writePlotLevel - got levelGrids");
  }
  ...
  {
    m_scalarNew[m_outputScalarVars[i]]->copyTo(m_scalarNew[i]->interval(),
                                               plotData, Interval(i, i));
  ...
  for (int i = 0; i < m_outputVectorVars.size(); i++)
  {
    ...
    m_vectorNew[m_outputVectorVars[i]]->copyTo(vecSrcComps, plotData,
                                               destComps);
  }
...
void AMRLevelMushyLayer::writePlotFile(int iter)
{
  ...
  char suffix[100];
  sprintf(suffix, "%06dlev%diter%d.%dd.hdf5", m_cur_step, m_level, iter,
          SpaceDim);
...
void AMRLevelMushyLayer::getExtraPlotFields()
{
  ...
  if (m_projection.isInitialized())
  {
    ...
    EdgeToCell(m_projection.MACcorrection(),
               *m_vectorNew[VectorVars::m_MACcorrection]);

    EdgeToCell(m_projection.grad_eLambda(),
               *m_vectorNew[VectorVars::m_freestreamCorrection]);

    Divergence::levelDivergenceMAC(*m_scalarNew[ScalarVars::m_divUcorr],
                                   m_projection.grad_eLambda(), m_dx);

    AMRLevelMushyLayer *coarsestML = this->getCoarsestLevel();
    Real coarsestDt = coarsestML->dt();

    for (DataIterator dit = m_scalarNew[ScalarVars::m_MACBC]->dataIterator();
         dit.ok(); ++dit)
    {
      ...
      (*m_vectorNew[VectorVars::m_MACcorrection])[dit].divide(m_dt /
                                                              coarsestDt);
    }
  }
}
...
void AMRLevelMushyLayer::computeVorticityStreamfunction()
{
  ...
  Real alpha = 0.0;
  Real beta = 1.0;
  //  op.define(m_problem_domain, m_grids, m_dx,
  //  m_physBCPtr->streamFunctionBC(), maxDepth, alpha, beta);

  op.define(lev0Dom, grids, refRat, lev0Dx, m_physBCPtr->streamFunctionBC(),
            alpha, beta);
  ...
  solver.define(lev0Dom, castFact, &bottomSolver, nLevels);
  ...
  solver.solve(amrPsi, amrVorticity, m_level, m_level,
...
void AMRLevelMushyLayer::computeVorticity()
{
  ...
  if (m_level > 0)
  {
    ...
    LevelData<FArrayBox> &crseVel =
        (*getCoarserLevel()->m_vectorNew[VectorVars::m_fluidVel]);
    const DisjointBoxLayout &crseGrids = crseVel.getBoxes();
    int nRefCrse = getCoarserLevel()->refRatio();

    QuadCFInterp interpolator(grids, &crseGrids, m_dx, nRefCrse, SpaceDim,
                              m_problem_domain);
  ...
  BCHolder vortVelBC =
      m_physBCPtr->extrapFuncBC(); // m_physBCPtr->extrapFuncBC(1);

  for (dit.reset(); dit.ok(); ++dit)
  {
    // apply physical BC's
    vortVelBC(vel[dit], grids[dit], m_problem_domain, m_dx,
              true); // homogeneous
    if (SpaceDim == 3)
    {
      for (int dir = 0; dir < SpaceDim; dir++)
      {
        // todo - 3D: vorticity needs more components in 3D, can't really store
        // it in m_scalarNew.
        // will need to make a new data structure which has the write number of
        // componenets for the dimensionality of the problem
    ...
    else if (SpaceDim == 2)
    {
      int dir = 2;
      FORT_COMPUTEVORT(
          CHF_FRA1((*m_scalarNew[ScalarVars::m_vorticity])[dit], 0),
          CHF_CONST_FRA(vel[dit]), CHF_BOX(grids[dit]), CHF_CONST_REAL(m_dx),
          CHF_CONST_INT(dir));
...
void AMRLevelMushyLayer::setDimensionlessReferenceEutectic()
{
  ...
  for (int dir = 0; dir < SpaceDim; dir++)
  {
    m_parameters.bcValBulkConcentrationHi[dir] =
        m_parameters.bcValBulkConcentrationHi[dir] - 1;
    m_parameters.bcValBulkConcentrationLo[dir] =
        m_parameters.bcValBulkConcentrationLo[dir] - 1;
...
void AMRLevelMushyLayer::setDimensionlessReferenceInitial()
{
  ...
  // Don't need to do anything if the reference point is already the initial
  // composition
  ...
  m_opt.refTemp =
      m_parameters.eutecticTemp -
      m_parameters.liquidusSlope *
          (m_parameters.eutecticComposition - m_parameters.initialComposition);
