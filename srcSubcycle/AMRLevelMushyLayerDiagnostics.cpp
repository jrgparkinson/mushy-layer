...
void AMRLevelMushyLayer::computeDiagnostics()
{
  ...
  // If a diagnostic period has been declared, check this time has passed since
  // we last produced diagnostics
  ...
  // Compute Nusselt number
  {
    ...
    if ((m_parameters.physicalProblem == PhysicalProblems::m_HRL ||
        ...
        m_level == 0)
    {
      fillScalars(*m_scalarNew[ScalarVars::m_temperature], m_time,
                  m_temperature);

      Real Nu =
          ::computeNusselt(*m_scalarNew[ScalarVars::m_temperature],
                           *m_vectorNew[VectorVars::m_fluidVel], m_dx,
                           m_parameters, m_opt.domainWidth, m_domainHeight);
             ...
             m_level == 0)
    {
      ...
      for (int lev = 0; lev < nLevels; lev++)
      {
        ...
        Tflux[lev]->define(ml->m_problem_domain, ml->m_grids, refRat[lev],
                           ml->m_dx, fineFR, coarseFR, SpaceDim);
        ...
        // Ensure BCs are set
        ml->fillScalars(*ml->m_scalarNew[ScalarVars::m_temperature], ml->m_time,
                        m_temperature, false, true);

        Gradient::levelGradientMAC(
            *gradEdgeT[lev], (*ml->m_scalarNew[ScalarVars::m_temperature]),
            ml->m_dx);
      ...
      // This makes sure we get the most accurate fluxes (using the finest level
      // available)
      ...
      m_diagnostics.addDiagnostic(DiagnosticNames::diag_NuRight, m_time,
                                  Nuright);
      ...
      m_diagnostics.addDiagnostic(DiagnosticNames::diag_NuMiddle, m_time,
                                  Nu_sum);
    ...
    if (m_diagnostics.diagnosticIsIncluded(DiagnosticNames::diag_maxUhalf))
    {
      ...
      // Have to do this ourselves, rather than using Chombo's computeMax(), as
      // we want to compute max over a single strip of cells
      ...
      m_diagnostics.addDiagnostic(DiagnosticNames::diag_maxVhalf, m_time,
                                  globalMaxHorizVel);
      m_diagnostics.addDiagnostic(DiagnosticNames::diag_maxUhalf, m_time,
                                  globalMaxVertVel);
    }
  }
  ...
  if (m_diagnostics.diagnosticIsIncluded(DiagnosticNames::diag_soluteFluxTop))
  {
    ...
    Vector<LevelData<FArrayBox> *> S_new, S_old, dSdt, H_new, horizAvFs,
        Fs_vert_diffusion, Fs_vert_fluid, Fs_vert_frame;
    ...
    for (int lev = 0; lev < nLevels; lev++)
    {
      ...
      horizAvFs[lev] = new LevelData<FArrayBox>(
          ml->m_grids, 1, IntVect::Zero); // no ghost vector
      Fs_vert_diffusion[lev] = new LevelData<FArrayBox>(
          ml->m_grids, 1, IntVect::Zero); // no ghost vector
      Fs_vert_fluid[lev] = new LevelData<FArrayBox>(
          ml->m_grids, 1, IntVect::Zero); // no ghost vector
      Fs_vert_frame[lev] = new LevelData<FArrayBox>(
          ml->m_grids, 1, IntVect::Zero); // no ghost vector

      ml->m_scalarNew[ScalarVars::m_averageVerticalFlux]->copyTo(
          *horizAvFs[lev]);
      ml->m_scalarNew[ScalarVars::m_FsVertDiffusion]->copyTo(
          *Fs_vert_diffusion[lev]);
    ...
    horizontallyAverage(averageVertFs,
    ...
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_Fs10, m_time,
                                averageVertFs[j_10]);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_Fs20, m_time,
                                averageVertFs[j_20]);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_Fs30, m_time,
                                averageVertFs[j_30]);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_Fs40, m_time,
                                averageVertFs[j_40]);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_Fs50, m_time,
                                averageVertFs[j_50]);

    // Another diagnostic - average vertical solute flux across the whole domain
    Real volume = 0.0;
    Real domainAverageFs =
        computeSum(volume, horizAvFs, refRat, lev0Dx, Interval(0, 0), 0);
    domainAverageFs = domainAverageFs / volume;
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_averageVerticalSaltFlux,
                                m_time, domainAverageFs);

    Real L2FsVertDiffusion =
        computeNorm(Fs_vert_diffusion, refRat, lev0Dx, Interval(0, 0), 2);
    Real L2FsVertFluid =
        computeNorm(Fs_vert_fluid, refRat, lev0Dx, Interval(0, 0), 2);
    Real L2FsVertFrame =
        computeNorm(Fs_vert_frame, refRat, lev0Dx, Interval(0, 0), 2);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L2FsVertDiffusion, m_time,
                                L2FsVertDiffusion);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L2FsVertFluid, m_time,
                                L2FsVertFluid);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L2FsVertFrame, m_time,
                                L2FsVertFrame);

    Real L1FsVertDiffusion =
        computeNorm(Fs_vert_diffusion, refRat, lev0Dx, Interval(0, 0), 1);
    Real L1FsVertFluid =
        computeNorm(Fs_vert_fluid, refRat, lev0Dx, Interval(0, 0), 1);
    Real L1FsVertFrame =
        computeNorm(Fs_vert_frame, refRat, lev0Dx, Interval(0, 0), 1);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L1FsVertDiffusion, m_time,
                                L1FsVertDiffusion);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L1FsVertFluid, m_time,
                                L1FsVertFluid);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L1FsVertFrame, m_time,
                                L1FsVertFrame);

    Real L0FsVertDiffusion =
        computeNorm(Fs_vert_diffusion, refRat, lev0Dx, Interval(0, 0), 0);
    Real L0FsVertFluid =
        computeNorm(Fs_vert_fluid, refRat, lev0Dx, Interval(0, 0), 0);
    Real L0FsVertFrame =
        computeNorm(Fs_vert_frame, refRat, lev0Dx, Interval(0, 0), 0);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L0FsVertDiffusion, m_time,
                                L0FsVertDiffusion);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L0FsVertFluid, m_time,
                                L0FsVertFluid);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_L0FsVertFrame, m_time,
                                L0FsVertFrame);
    ...
    Real Fs_bottom =
        m_saltDomainFluxRegister.getFluxHierarchy(1, Side::Lo, 1.0);
    Real Fs_top = m_saltDomainFluxRegister.getFluxHierarchy(1, Side::Hi, 1.0);

    Real Fh_bottom =
        m_heatDomainFluxRegister.getFluxHierarchy(1, Side::Lo, 1.0);
    ...
    {
      Real Fs_hi =
          m_saltDomainFluxRegister.getFluxHierarchy(dir, Side::Hi, 1.0);
      Real Fs_lo =
          m_saltDomainFluxRegister.getFluxHierarchy(dir, Side::Lo, 1.0);

      Real Fh_hi =
          m_heatDomainFluxRegister.getFluxHierarchy(dir, Side::Hi, 1.0);
      Real Fh_lo =
          m_heatDomainFluxRegister.getFluxHierarchy(dir, Side::Lo, 1.0);
    ...
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_soluteFluxBottom, m_time,
                                totalF_bottom);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_soluteFluxTop, m_time,
                                totalF_top);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_heatFluxAbsMismatch,
                                m_time, heat_mismatch);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_saltFluxAbsMismatch,
                                m_time, salt_mismatch);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_heatFluxRelMismatch,
                                m_time, rel_heat_mismatch);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_saltFluxRelMismatch,
                                m_time, rel_salt_mismatch);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_heatFluxBottom, m_time,
                                totalFh_bottom);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_heatFluxTop, m_time,
                                totalFh_top);
      ...
      m_diagnostics.diagnosticIsIncluded(DiagnosticNames::diag_maxLambda))
  {
    ...
    Real sumLambda =
        ::computeNorm(amrLambda, nRef, m_dx, Interval(0, 0), 2, m_level);
    Real maxLambda = ::computeNorm(
        amrLambda, nRef, m_dx, Interval(0, 0), 0,
        m_level); //::computeSum(amrLambda,nRef,m_dx, Interval(0,0), m_level);

    Real maxFreestreamCorr =
        ::computeNorm(*m_vectorNew[VectorVars::m_freestreamCorrection], nullptr,
                      1, m_dx, Interval(0, SpaceDim - 1), 0);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_maxFreestreamCorrection,
                                m_time, maxFreestreamCorr);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_maxLambda, m_time,
                                maxLambda);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_sumLambda, m_time,
                                sumLambda);
      ...
      m_diagnostics.diagnosticIsIncluded(DiagnosticNames::diag_avSalinity))
  {
    ...
    horizontallyAverage(averagedSalinity,
    ...
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_HorizAvSalinity0, m_time,
                                averagedSalinity[y_0]);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_HorizAvSalinity20, m_time,
                                averagedSalinity[y_1]);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_HorizAvSalinity40, m_time,
                                averagedSalinity[y_2]);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_HorizAvSalinity60, m_time,
                                averagedSalinity[y_3]);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_avSalinity, m_time,
                                averageSl);
  }
  ...
  if (calcDiagnostics && m_diagnostics.diagnosticIsIncluded(
                             DiagnosticNames::diag_mushyAverageBulkConc))
  {

    Real mushVol;
    Real mushAvBulkC =
        averageOverMushyRegion(ScalarVars::m_bulkConcentration, mushVol);
    //    Real mushAvBulkC = 0.0;
    Real mushAvPorosity =
        averageOverMushyRegion(ScalarVars::m_porosity, mushVol);

    // Convert mush volume in number of cells to physical volume (in
    // dimensionless units)
    mushVol *= pow(m_dx, SpaceDim);

    m_diagnostics.addDiagnostic(DiagnosticNames::diag_mushyAverageBulkConc,
                                m_time, mushAvBulkC);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_mushyAveragePorosity,
                                m_time, mushAvPorosity);
    m_diagnostics.addDiagnostic(DiagnosticNames::diag_mushyVol, m_time,
                                mushVol);
  }
  ...
  // - how big
  // - what spacing
  // Commented out for now as very slow
  // if (m_level == 0)
  // {
