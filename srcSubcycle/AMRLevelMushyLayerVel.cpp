...
void AMRLevelMushyLayer::calculateTimeIndAdvectionVel(
    Real time, LevelData<FluxBox> &a_advVel)
{
  ...
  RefCountedPtr<LevelData<FluxBox>> crsePressureScaleEdgePtr,
      pressureScaleEdgePtr;
  ...
  LevelData<FluxBox> *velocityBCVals =
      new LevelData<FluxBox>(m_grids, 1, a_advVel.ghostVect());
  ...
  // Construct a fluxbox containing values we want to enforce at the boundary
  // (don't usually end up using this)
  for (DataIterator dit = velocityBCVals->dataIterator(); dit.ok(); ++dit)
  {
    ...
    // Setting BC for vertical velocity (component = 1) in vertical direction
    // (idir=1) on low side.
    //    int velComp = 1;
  ...
  EdgeVelBCHolder edgeVelBC(
      m_physBCPtr->edgeVelFuncBC(m_opt.viscousBCs, velocityBCVals));
  ...
  // if a coarser level exists, will need coarse-level data for the projection
  // boundary conditions
  if (m_level > 0)
  {
    AMRLevelMushyLayer *amrMLcrse = getCoarserLevel();
    const DisjointBoxLayout &crseGrids = amrMLcrse->m_grids;
    crsePressurePtr = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(crseGrids, 1));

    amrMLcrse->fillScalars(*crsePressurePtr, time, ScalarVars::m_pressure,
                           true);

    if (m_opt.scaleP_MAC)
    {
      crsePressureScaleEdgePtr = RefCountedPtr<LevelData<FluxBox>>(
          new LevelData<FluxBox>(amrMLcrse->m_grids, 1, IntVect::Unit));
      amrMLcrse->fillScalarFace(*crsePressureScaleEdgePtr, time,
                                m_pressureScaleVar, true);

      crsePressureScalePtr = RefCountedPtr<LevelData<FArrayBox>>(
          new LevelData<FArrayBox>(amrMLcrse->m_grids, 1, IntVect::Unit));
      amrMLcrse->fillScalars(*crsePressureScalePtr, time, m_pressureScaleVar,
                             true);
    }
  }
  ...
  if (m_opt.scaleP_MAC)
  {
    pressureScaleEdgePtr = RefCountedPtr<LevelData<FluxBox>>(
        new LevelData<FluxBox>(m_grids, 1, IntVect::Unit));
    fillScalarFace(*pressureScaleEdgePtr, time, m_pressureScaleVar, true);

    pressureScalePtr = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(m_grids, 1, IntVect::Unit));
  ...
  if (m_parameters.isViscous() && !m_opt.explicitViscousVelSolve)
  {
    LOG_DEBUG("Viscous advection velocity solve (level " << m_level << ")");

    LevelData<FArrayBox> uDelu(m_grids, SpaceDim,
                               this->m_numGhostAdvection * IntVect::Unit);
    setValLevel(uDelu, 0.0);

    LevelData<FArrayBox> advectionSourceTerm(
        m_grids, SpaceDim, m_numGhostAdvection * IntVect::Unit);
    setValLevel(advectionSourceTerm, 0.0);
    this->computeAdvectionVelSourceTerm(advectionSourceTerm);
    advectionSourceTerm.exchange();
    uDelu.exchange();

    this->computeUstar(uDelu, advectionSourceTerm, m_time - m_dt, m_dt, true,
                       true,
                       false); // no u.del(u) term

    // Average U^* to faces for projection

    // not sure if these exchange calls are necessary, could possibly remove to
    // speed up code
    m_vectorNew[VectorVars::m_advUstar]->exchange();
    CellToEdge(*m_vectorNew[VectorVars::m_advUstar],
               a_advVel); // for MAC projection option in computeUstar
    a_advVel.exchange();
  }
  else
  {
    LOG_DEBUG("Inviscid advection velocity solve (level " << m_level << ")");
    ...
    //    m_opt.useIncrementalPressure;
    bool useIncrementalPressure =
        (m_level == 0) && m_opt.useIncrementalPressure;
    ...
    if (useIncrementalPressure)
    {
      ...
      Real maxPressure = ::computeNorm(m_projection.phi(), nullptr, 1, m_dx,
                                       Interval(0, 0), 0);
      // This is chosen empirically and may need some refinement
      Real pressureCap = 1e50;

      // A better indicator of issues arising is that the pressure becomes
      // negative as we fix P = 0 on the bottom boundary, and it should increase
      // up through the domain, use this condition instead
      Real minPressure =
          ::computeMin(m_projection.phi(), nullptr, 1, Interval(0, 0));
      if (minPressure > 0 && maxPressure < pressureCap)
      {
        ...
        m_projection.applyMacCorrection(a_advVel, nullptr, phiScale);

        // If we're on refined levels, and we've subtracted off the old
        // pressure, then the boundary condition on coarse fine interface is no
        // longer the full pressure, but is the pressure minus what we've just
        // subtracted off - make that change here
        // todo - cf boundary condition for pressure with incremental update
        if (m_level > 0)
        {
          LevelData<FArrayBox> coarsenedPressure(
              crsePressurePtr->disjointBoxLayout(), 1,
              crsePressurePtr->ghostVect());

          setValLevel(coarsenedPressure, 0.0);

          // Make coarsened pressure
          m_coarseAverageScalar.averageToCoarse(coarsenedPressure,
                                                m_projection.phi());

          for (DataIterator dit = crsePressurePtr->dataIterator(); dit.ok();
               ++dit)
          {
  ...
  while (maxDivU > 1e-10 && proj_i <= maxNumProj)
  {
    ...
    LOG_DEBUG("Project advection velocity (level " << m_level << ")");

    // Copy to U^* before projection
    EdgeToCell(a_advVel, *m_vectorNew[VectorVars::m_advUstar]);

    a_advVel.exchange();
    int exitStatus = m_projection.levelMacProject(
        a_advVel, m_dt, crsePressurePtr, pressureScalePtr, crsePressureScalePtr,
        pressureScaleEdgePtr, crsePressureScaleEdgePtr, alreadyHasPressure,
        correctScale);
    ...
    Divergence::levelDivergenceMAC(*m_scalarNew[ScalarVars::m_divUadv],
                                   a_advVel, m_dx);
    maxDivU = ::computeNorm(*m_scalarNew[ScalarVars::m_divUadv], nullptr, 1,
                            m_dx, Interval(0, 0), 0);
    Real minPressure =
        ::computeMin(m_projection.phi(), nullptr, 1, Interval(0, 0));
    LOG_INFO("  MAC Projection (level " << m_level
  ...
  //  bool m_opt.enforceAnalyticVel = (m_parameters.physicalProblem ==
  //  PhysicalProblems::m_soluteFluxTest  );
  ...
  LOG_DEBUG("Apply freestream correction to advection velocity (level "
            << m_level << ")");

  // Finally - apply freestream preservation correction from previous
  // synchronisation step
  m_projection.applyFreestreamCorrection(a_advVel);

  // Face centred velocity field should be divergence free (to machine
  // precision)
  Divergence::levelDivergenceMAC(*m_scalarNew[ScalarVars::m_divUadv], a_advVel,
                                 m_dx);

  // Need a m_fluidVel to be populated to some advection things the right way
  // around
  EdgeToCell(m_advVel, *m_vectorNew[VectorVars::m_fluidVel]);
  this->fillVectorField(*m_vectorNew[VectorVars::m_fluidVel], time,
                        VectorVars::m_fluidVel);

  // CLean up
  if (velocityBCVals != nullptr)
  {
    delete velocityBCVals;
    velocityBCVals = nullptr;
  }

  LOG_FUNCTION_EXIT();
}

void AMRLevelMushyLayer::computeAdvectionVelocities(
    LevelData<FArrayBox> &advectionSourceTerm, Real advVelCentering)
{
  ...
  RefCountedPtr<LevelData<FluxBox>> porosityFaceAvPtr =
      RefCountedPtr<LevelData<FluxBox>>(
          new LevelData<FluxBox>(m_grids, 1, ivGhost));
  ...
  {
    LOG_DEBUG("Upwinding velocities to compute advection velocities");
     ...
     *    solve d(u/porosity)/dt + (u/porosity).grad(u/porosity) = S/porosity
     * (-(u/porosity^2)dporosity/dt) (assume d(porosity)/dt = 0)
     * m_porosityOutsideAdvection:
     *    solve du/dt + (u/porosity).grad(u) = S +
     * (u.grad(porosity)/porosity^2)u m_noPorosity: solve du/dt + u.grad(u) = S
     */
    ...
    for (DataIterator dit = U_to_advect.dataIterator(); dit.ok(); ++dit)
    {
      ...
      if (m_opt.advectionMethod == m_porosityInAdvection)
      {
        ...
        {
          U_to_advect[dit].divide(porosityGrown[dit], porosityGrown[dit].box(),
                                  0, dir, 1);
        }
      }
          ...
          m_opt.advectionMethod == m_porosityInAdvection)
      {
        ...
        m_advVel[dit].divide((*porosityFaceAvPtr)[dit], m_advVel[dit].box(), 0,
                             0);
        for (int dir = 0; dir < SpaceDim; dir++)
        {
    ...
    if (cfl > 1.0)
    {
      LOG_INFO("WARNING - about to do trace adv vel with cfl = " << cfl);
    }

    //        LOG_INFO("Do trace advection vel with cfl = " << cfl);

    edgeVelBC.applyBCs(m_advVel, m_grids, m_problem_domain, m_dx,
                       false); // inhomogeneous

    // m_dt is the full timestep, and this always returns the half time velocity
    // changed this from m_dt (full timestep) to half_dt*2 incase we want to
    // compute velocities at some odd point in time
    ...
    Real maxAdvU = ::computeNorm(*m_vectorNew[VectorVars::m_advectionVel],
                                 nullptr, 1, m_dx, Interval(0, 0));

    if (s_verbosity >= 1 && maxAdvU > 1e10)
    {
      LOG_INFO(
          "WARNING - max advection velocity (pre-projection) = " << maxAdvU);
    }
    ...
    if (m_opt.advectionMethod == m_porosityInAdvection)
    {
      ...
      {
        m_advVel[dit].mult((*porosityFaceAvPtr)[dit], m_advVel[dit].box(), 0,
                           0);
      }
    }
    ...
    // this is necessary to remove some \delta x scale instabilities which seem
    // to arise
    ...
    Real project_dt = 2 * half_dt; // this usually just works out at m_dt, for
                                   // time centred advection velocities
    ...
    // Maybe replace m_advVel with time independent version in low porosity
    // regions?

    if (m_opt.chiLimit > 0.0)
    {
      LevelData<FluxBox> mushyAdvVel(m_advVel.disjointBoxLayout(), 1,
                                     m_advVel.ghostVect());
      ...
      LOG_INFO("  ComputeAdvectionVelocities - replaced "
               << count << " cells with darcy flow");

      // Project again
      correctEdgeCentredVelocity(m_advVel, project_dt);
    }

    // The projection solve isn't always quite good enough, which can leave
    // U~1e-10 where porosity ~1e-15 want to avoid u/chi > 1 for porosity
    // =1e-15, so do it manually
    //    setVelZero(m_advVel, advPorosityLimit);

    LevelData<FluxBox> porosityFace(m_advVel.disjointBoxLayout(), 1,
                                    m_advVel.ghostVect());
  ...
  {
    // not doing velocity advection

    LOG_DEBUG(
        "computeAdvectionVelocities - computing time independent velocities");

    // if we're not doing advection of velocities, calculate the velocities for
    // scalar advection by averaging our CC velocities to faces at the half time
  ...
  if (m_opt.enforceAnalyticVel)
  {
    ...
    LevelData<FluxBox> velDiff(m_advVel.disjointBoxLayout(), 1,
                               m_advVel.ghostVect());
  ...
  Divergence::levelDivergenceMAC(*m_scalarNew[ScalarVars::m_divUadv], m_advVel,
                                 m_dx);
...
void AMRLevelMushyLayer::removeGradPressure(Real a_oldTime, Real a_dt,
                                            bool a_MACprojection, int uvar)
{
  ...
  // first get gradPi
  fillPressureSrcTerm(gradP, pressureScale, a_oldTime - a_dt / 2,
                      a_MACprojection);
...
void AMRLevelMushyLayer::computeCCvelocity(
    const LevelData<FArrayBox> &advectionSourceTerm, Real a_oldTime, Real a_dt,
    bool doFRupdates, bool a_doProjection, bool compute_uDelU,
    bool a_MACprojection)
{
  ...
  // Calculate Ustar
  LOG_DEBUG("Compute U^* (level " << m_level << ")");

  computeUstar(*m_vectorNew[VectorVars::m_UdelU], advectionSourceTerm,
               a_oldTime, a_dt, doFRupdates, a_MACprojection, compute_uDelU);

  Real maxUstar = ::computeNorm(*m_vectorNew[ustarVar], nullptr, 1, m_dx,
                                Interval(0, 0), 0);
  if (s_verbosity >= 1 && maxUstar > 1e10)
  {
    LOG_INFO(
        "WARNING - max cell-centred velocity (pre-projection) = " << maxUstar);
  }
  ...
  if (m_opt.scaleP_CC)
  {
    ...
    pressureScaleEdgePtr =
        RefCountedPtr<LevelData<FluxBox>>(new LevelData<FluxBox>(m_grids, 1));

    // Need a ghost cell here for 5 point stencil
    pressureScalePtr = RefCountedPtr<LevelData<FArrayBox>>(
        new LevelData<FArrayBox>(m_grids, 1, IntVect::Unit));

    fillScalarFace(*pressureScaleEdgePtr, half_time, m_pressureScaleVar,
                   arithmeticAveraging, true, false, porositySmoothing);
  ...
  if (m_level > 0)
  {
    ...
    {
      // If we're not doing +/- grad(P) stuff then just pass in the velocity
      // before it was projected else the divergence at coarse-fine boundaries
      // will be crazy
    ...
    {
      crsePressureScaleEdgePtr = RefCountedPtr<LevelData<FluxBox>>(
          new LevelData<FluxBox>(amrMLcrse->m_grids, 1));
      amrMLcrse->fillScalarFace(*crsePressureScaleEdgePtr, half_time,
                                m_pressureScaleVar, true, porositySmoothing);

      // Need a ghost cell for 5 point stencil
      crsePressureScalePtr = RefCountedPtr<LevelData<FArrayBox>>(
          new LevelData<FArrayBox>(amrMLcrse->m_grids, 1, IntVect::Unit));
      amrMLcrse->fillScalars(*crsePressureScalePtr, half_time,
                             m_pressureScaleVar, true);
    }
  }
  ...
  velBC.applyBCs(*m_vectorNew[uvar], m_grids, m_problem_domain, m_dx,
                 false); // inhomogeneous
  ...
  if (m_opt.doProjection && a_doProjection)
  {
    ...
    QuadCFInterp interp;
    Divergence::levelDivergenceCC(*m_scalarNew[ScalarVars::m_divU],
                                  *m_vectorNew[uvar], nullptr, m_dx, true,
                                  interp);

    Real initMaxDivU = computeNorm(*m_scalarNew[ScalarVars::m_divU], nullptr,
                                   -1, m_dx, Interval(0, 0), 0);
    LOG_DEBUG("Cell centred projection init max(div(U)) = " << initMaxDivU);

    Real maxDivU = initMaxDivU;
    Real relTol = 1e-5;
    int i = 0;

    // allow mulitple projections on level 0 (can't do this on refined levels as
    // it breaks the CF boundary conditions)
    int maxNumProj = 1;
    if (m_level == 0)
    {
    ...
    while (!(maxDivU < initMaxDivU * relTol || maxDivU < relTol ||
             i >= maxNumProj))
    {
      ...
      {
        m_projection.LevelProject(
            *m_vectorNew[uvar], crseVelPtr, new_time, a_dt, pressureScalePtr,
            crsePressureScalePtr, pressureScaleEdgePtr,
            crsePressureScaleEdgePtr, m_parameters.isViscous());
      }
      ...
      maxDivU = computeNorm(*m_scalarNew[ScalarVars::m_divU], nullptr, -1, m_dx,
                            Interval(0, 0), 0);

      LOG_INFO("  CCProjection: max(div(U)) = " << maxDivU);
  ...
  //  Real max = ::computeMax(U_chi, nullptr, 1, Interval(0, SpaceDim-1));
  //  if (max > 1e5)
  //  {
  //    LOG_INFO("U_chi > 1e5!!! ");
  //  }
  ...
  //  QuadCFInterp interp;
  //  Divergence::levelDivergenceCC(*m_scalarNew[ScalarVars::m_divU],
  //  *m_vectorNew[uvar], nullptr, m_dx, true, interp); Real maxDivU =
  //  computeNorm(*m_scalarNew[ScalarVars::m_divU], nullptr, -1, m_dx,
  //  Interval(0,0), 0);
...
void AMRLevelMushyLayer::computeLapVel(
    LevelData<FArrayBox> &a_lapVel, LevelData<FArrayBox> &a_vel,
    const LevelData<FArrayBox> *a_crseVelPtr)
{

  // These are set in applyOpI anyway, but do it again here so we can see what
  // happens
  VelBCHolder velBC(m_physBCPtr->uStarFuncBC(m_opt.viscousBCs));
  velBC.applyBCs(a_vel, a_vel.disjointBoxLayout(), m_problem_domain, m_dx,
                 false);

  // Copy velocity to new data holder before taking laplacian to avoid changing
  // original velocity
  LevelData<FArrayBox> a_vel_copy(
      m_grids, SpaceDim,
      2 * IntVect::Unit); // 2 ghost vectors for higher order laplace
  DataIterator dit(m_grids);
  ...
  a_vel.copyTo(Interval(0, SpaceDim - 1), a_vel_copy,
  ...
  if (a_crseVelPtr)
  {
    ...
    //          extrapBC.applyBCs(a_vel_copy, a_vel_copy.disjointBoxLayout(),
    //          m_problem_domain, m_dx, false);
  }

  // Apply domain BCs again?
  velBC.applyBCs(a_vel_copy, a_vel_copy.disjointBoxLayout(), m_problem_domain,
                 m_dx, false);
  ...
  for (int dir = 0; dir < SpaceDim; dir++)
  {
    ...
    // todo - Nonurgent: should be possible to replace this with the Darcy
    // Brinkman Op, to save having the viscous op lying around
    ...
    //          IntVect iv = bit();
    //
    //          thisLap(iv, dir) = (1-m_opt.lapVelSmoothScale)*thisLap(iv, dir)
    //          + (m_opt.lapVelSmoothScale/4)*(thisLap(iv+BASISV(0), dir) +
  ...
  // may need to extend lapVel to cover ghost cells as well
  {
    ...
    if (m_opt.lapVelBCOrder >= 0)
    {
      ...
      for (dit.reset(); dit.ok(); ++dit)
      {
        bc(a_lapVel[dit], grids[dit], m_problem_domain, m_dx,
...
void AMRLevelMushyLayer::computePredictedVelocities(
    ...
    PatchGodunov &a_patchGodVelocity, LevelData<FluxBox> &a_grad_eLambda,
    LevelData<FluxBox> &a_gradPhi, LevelData<FluxBox> &porosityFace,
    Real a_old_time, Real a_dt)
{

  // Get AdvectionPhysics object within the PatchGodunov object
  AdvectionPhysics *advectionPhysics = dynamic_cast<AdvectionPhysics *>(
      a_patchGodVelocity.getGodunovPhysicsPtr());
  if (advectionPhysics == nullptr)
  {
    MayDay::Error("AMRLevelMushyLayer::predictVelocities - unable to upcast "
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    ...
    // u_chi_predicted (and this_U_chi_dir) should contain whatever we have
    // advected (usually u/chi). meanwhile, thisAdvVel (and thisAdvVelDir)
    // contains whatever we were advecting with (usually u).
    ...
    for (int velComp = 0; velComp < SpaceDim; velComp++)
    {
      for (int dir = 0; dir < SpaceDim; dir++)
      {
        ...
        else
        {
          ...
          // add MAC correction to traced velocities so they are (roughly)
          // divergence free if we are advecting u/chi, need to divide
          // correction by chi too

          // The MAC correction computed earlier in this timestep
          FArrayBox &gradPhiDir = a_gradPhi[dit()][dir];

          // Scale correction with chi too
          // as the correction was computed for U, and we're in fact correcting
          // U/chi
          gradPhiDir.divide(porosityDir, 0, dir, 1);

          // subtract correction
          this_U_chi_Dir.minus(gradPhiDir, velComp, velComp, 1);
...
void AMRLevelMushyLayer::computeUstar(
    LevelData<FArrayBox> &a_UdelU,
    const LevelData<FArrayBox> &advectionSourceTerm, Real a_oldTime, Real a_dt,
    bool doFRupdates, bool a_MACprojection, bool compute_uDelU)
{
  ...
  computeUstarSrc(src, advectionSourceTerm, src_time, a_MACprojection,
                  compute_uDelU);
  ...
  {
    LOG_DEBUG("Do viscous solve (level " << m_level << ")");

    /*
     * multi component solve not implemented yet. Currently we solve in each
     * direction separately, but it might be faster if the solver could do both
     * simultaneously, to avoid lots of extra multigrid coarsening and
     * refinement. One to look at in the future.
    ...
    if (m_opt.multiCompUStarSolve)
    {
      ...
      //        Real lev0Dx;
      //        getHierarchyAndGrids(hierarchy, allGrids, refRat, lev0Dom,
      //        lev0Dx);
      //
      //        // Make these thingsmulticomp
      //        defineUstarMultigrid();
      //        UstarBE.define(allGrids, refRat, lev0Dom,
      //        s_uStarOpFactMultiComp, s_uStarAMRMGMultiComp);
      //        UstarTGA.define(allGrids, refRat, lev0Dom,
      //        s_uStarOpFactMultiComp, s_uStarAMRMGMultiComp);
    }
    else
    {
      ...
      // Do the solve for each component separately (as the solver is hardwired
      // for one component only)
      LevelData<FArrayBox> UstarCrse, UoldCrse, *UstarCrseComp, *UoldCrseComp,
          Uold;
      LevelData<FluxBox> diffusiveFlux(m_grids, SpaceDim);
      LevelFluxRegister *fineFluxRegPtr = nullptr, *crseFluxRegPtr = nullptr;
      ...
      UstarCrseComp = nullptr;
      UoldCrseComp = nullptr;
      ...
      if (m_level > 0)
      {
        ...
        //      amrMLcrse->fillVectorField(UstarCrse, new_crseTime, m_Ustar,
        //      true); amrMLcrse->fillVectorField(UoldCrse, old_crseTime,
        //      m_Ustar, true);

        // Coarse boundary conditions should be either
        //    a) the fully projected velocity on the coarse level in we're
        //    including grad(p) in the source
        ...
        crseFluxRegPtr =
            &(*(amrMLcrse->m_vectorFluxRegisters[VectorVars::m_fluidVel]));
      }
      ...
      for (int comp = 0; comp < SpaceDim; comp++)
      {
        ...
        if (m_level > 0)
        {
          ...
          rhsVectComp.push_back(nullptr); // this isn't used
        }
        ...
        if (!doFRupdates)
        {
          crseFluxRegPtr = nullptr;
          fineFluxRegPtr = nullptr;
        }
        ...
        if (this->isVelocityTimeDependent())
        {
          ...
          if (m_opt.timeIntegrationOrder == 1)
          {
            UstarBE[comp]->updateSoln(UstarComp, UoldComp, compSrc,
                                      ...
                                      UoldCrseComp, UstarCrseComp, old_time,
                                      old_crseTime, new_crseTime, a_dt, m_level,
                                      false, comp); // False - don't zero phi
          ...
          {
            UstarTGA[comp]->updateSoln(
                UstarComp, UoldComp, compSrc, fineFluxRegPtr, crseFluxRegPtr,
                UoldCrseComp, UstarCrseComp, old_time, old_crseTime,
                new_crseTime, a_dt, m_level, false,
                comp); // False - don't zero phi
#ifdef CH_FORK
        ...
        {
          // need to implement non time dependent solve

          // might need to fix up lmax min
          m_uStarAMRMG[comp]->solve(UstarVectComp, rhsVectComp, 0, 0, false,
                                    false);
        }
      ...
      // Clean up
      if (UstarCrseComp != nullptr)
      {
        delete UstarCrseComp;
        UstarCrseComp = nullptr;
      }
      if (UoldCrseComp != nullptr)
      {
        delete UoldCrseComp;
        UoldCrseComp = nullptr;
      }
...
void AMRLevelMushyLayer::computeAdvectionVelSourceTerm(
    LevelData<FArrayBox> &a_src)
{
  ...
  LevelData<FArrayBox> velOld(m_grids, SpaceDim,
                              m_numGhostAdvection * IntVect::Unit);
  LevelData<FArrayBox> permeability(m_grids, 1,
                                    m_numGhostAdvection * IntVect::Unit);
  ...
  if (m_level > 0)
  {
    crseVelPtr = new LevelData<FArrayBox>(getCoarserLevel()->m_grids, SpaceDim,
                                          IntVect::Unit);
  ...
  fillPressureSrcTerm(
      pressure, pressureScale, src_time - m_dt / 2,
      false); // false - make sure we use Pi (true means use phi)

  // Make copy of these options in case we want to change them for just this
  // timestep
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    ...
    for (int dir = 0; dir < SpaceDim; dir++)
    {
      darcy[dit].divide(permeability[dit], darcy[dit].box(), 0, dir);

      darcy[dit].mult(porosity[dit], darcy[dit].box(), 0, dir);
  ...
  if (m_opt.advectionMethod == m_porosityOutsideAdvection)
  {
    ...
    Gradient::levelGradientCC(extraSrc, porosity, m_dx,
  ...
  Real maxSrcVal = ::computeNorm(a_src, nullptr, 1, m_dx);

  if (maxSrcVal > 1e10)
  {
    LOG_INFO("WARNING - norm of advection velocity src term = " << maxSrcVal);
  }
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    ...
    (*m_vectorOld[VectorVars::m_advectionSrc])[dit].copy(a_src[dit], 0, 0,
                                                         SpaceDim);

    // Also need to store in vectorNew so that we can interpolate in time when
    // subcycling
    (*m_vectorNew[VectorVars::m_advectionSrc])[dit].copy(a_src[dit], 0, 0,
                                                         SpaceDim);
  }
...
void AMRLevelMushyLayer::computeUDelU(
    LevelData<FArrayBox> &U_adv_src,
    const LevelData<FArrayBox> &advectionSourceTerm, Real a_oldTime,
    Real a_dt)
{
  // May want to use pre existing value for this, rather than re computing.

  LevelData<FArrayBox> UdelU_porosity(m_grids, SpaceDim,
                                      m_numGhostAdvection * IntVect::Unit);
  ...
  if (doVelocityAdvection())
  {
    ...
    // option 0 - use advection velocity to upwind old velocities to cell faces,
    // and use these in calculation option 1 - average advection velocities to
    // cell centers, and use this along with gradient of advection velocities to
    // computer u.del(u)
    if (m_opt.uDeluMethod == 0)
    {
      bool doVelFRupdates = true;
      predictVelocities(UdelU_porosity, m_advVel, advectionSourceTerm,
                        a_oldTime, a_dt, doVelFRupdates);
    }
    else if (m_opt.uDeluMethod == 1)
    {
      ...
      Real maxUdelU =
          ::computeMax(UdelU_porosity, nullptr, 1, Interval(0, SpaceDim - 1));

      if (maxUdelU > 1e3)
      {
        LOG_INFO("max(uDelU) = " << maxUdelU);
      }
    }
  }
  else
  ...
  setVelZero(UdelU_porosity, m_opt.uDelU_porosityLimit,
             m_opt.uDelU_grow); // grow by 1

  UdelU_porosity.copyTo(U_adv_src);

  for (dit.reset(); dit.ok(); ++dit)
  {
    (*m_vectorNew[VectorVars::m_UdelU])[dit].copy(UdelU_porosity[dit], 0, 0,
                                                  SpaceDim);
  }
}

void AMRLevelMushyLayer::computeUstarSrc(
    LevelData<FArrayBox> &src, const LevelData<FArrayBox> &advectionSourceTerm,
    Real src_time, bool a_MACprojection, bool compute_uDelU)
{
  ...
  if (m_level > 0)
  {
    ...
    crseOldVel.define(amrMLcrse->m_grids, SpaceDim, srcGhost * IntVect::Unit);
  ...
  IntVect ivGhost = srcGhost * IntVect::Unit;
  LevelData<FArrayBox> U_src(
      m_grids, SpaceDim,
      ivGhost);                                                      // = (1 - dt pi_0 chi / (2*pi)  +(Da dt/2) laplacian) u^n
  LevelData<FArrayBox> U_nonviscous_src(m_grids, SpaceDim, ivGhost); //
  LevelData<FArrayBox> P_src(m_grids, SpaceDim,
                             ivGhost); // = chi * grad(P^{n-1/2})
  LevelData<FArrayBox> buoyancy_src(
      m_grids, SpaceDim, ivGhost); // = chi*Ra*(theta^n+theta^{n+1})/2 e_z
  LevelData<FArrayBox> darcy_src(m_grids, SpaceDim,
                                 ivGhost); // = (pr/Da) * chi*U/Pi
  LevelData<FArrayBox> U_correction(m_grids, SpaceDim,
                                    ivGhost); //= (1-acoef)*U_old to correct for
                                              //how TGA and backward euler work
  ...
  fillScalars(liquidConcentration_centred, src_time, m_liquidConcentration,
              true);
  fillScalars(pressureScale, src_time, m_pressureScaleVar, true);

  // Now the Pressure bit
  fillPressureSrcTerm(P_src, pressureScale, oldTime - m_dt / 2,
                      a_MACprojection);

  // Buoyancy
  fillBuoyancy(buoyancy_src, temperature_centred, liquidConcentration_centred,
               porosity_centred);
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    ...
    for (int dir = 0; dir < SpaceDim; dir++)
    {
      ...
      darcy_src[dit].divide(permeability_centred[dit], darcy_src[dit].box(), 0,
                            dir, 1);
      // todo: add viscosity in here
    }
                                           ...
                                           bool doFRupdates)
{
  ...
  // tracing uses inviscid BC's
  {
    ...
    velBC.applyBCs(traceVel, levelGrids, m_problem_domain, m_dx,
                   false); // not homogeneous
  }
  ...
  for (DataIterator dit = a_advVel.dataIterator(); dit.ok(); ++dit)
  {
             ...
             m_opt.advectionMethod == m_porosityInAdvection)
    {
      ...
      advectionVelocity[dit].divide(porosityFace[dit], porosityFace[dit].box(),
                                    0, 0);
      for (int dir = 0; dir < SpaceDim; dir++)
      {
  ...
  // This is the half time predicted value of whatever we were advecting (u or
  // u/chi)
  ...
  { // this is to allow this fluxbox to go out of scope
    ...
    if (m_opt.scaleP_MAC)
    {
      for (DataIterator dit = gradPhi.dataIterator(); dit.ok(); ++dit)
      {
        for (int dir = 0; dir < SpaceDim; dir++)
        {
          gradPhi[dit].mult(pressureScale[dit], pressureScale[dit].box(), 0,
                            dir);
        }
      }
    }
    ...
    // This should U_chi_advected - if we've only advected U, then it should
    // divide by chi before returning
    ...
    for (DataIterator dit = U_chi_advected.dataIterator(); dit.ok(); ++dit)
    {
      for (int dir = 0; dir < SpaceDim; dir++)
      {
        EdgeToCell(U_chi_advected[dit], dir,
                   (*m_vectorNew[VectorVars::m_U_porosity])[dit], 0, dir);
      }
    }
  ...
  // cellAdvVel is U_advection averaged to cells. Never any factors of chi in
  // here.
  ...
  LevelData<FluxBox> momentumFlux(U_chi_advected.disjointBoxLayout(), SpaceDim,
                                  U_chi_advected.ghostVect());
  U_chi_advected.copyTo(Interval(0, SpaceDim - 1), momentumFlux,
                        Interval(0, SpaceDim - 1));
        ...
        for (int dir = 0; dir < SpaceDim; dir++)
        {
          ...
          if (m_opt.reflux_normal_momentum)
          {
            m_vectorFluxRegisters[reflux_var]->incrementCoarse(
                momentumFlux[dit][dir], fluxMult, dit(), velComps, velComps,
                dir);
          }
          else
          {
            ...
            for (int velComp = 0; velComp < SpaceDim; velComp++)
            {
              if (velComp != dir)
              {
                ...
                m_vectorFluxRegisters[reflux_var]->incrementCoarse(
                    momentumFlux[dit][dir], fluxMult, dit(), velInt, velInt,
                    dir);
          ...
          else
          {
            ...
            for (int velComp = 0; velComp < SpaceDim; velComp++)
            {
              if (velComp != dir)
              {
                ...
                crseFR.incrementFine(momentumFlux[dit][dir], fluxMult, dit(),
                                     velInt, velInt, dir, Side::Lo);

                crseFR.incrementFine(momentumFlux[dit][dir], fluxMult, dit(),
                                     velInt, velInt, dir, Side::Hi);
...
void AMRLevelMushyLayer::traceAdvectionVel(
    LevelData<FluxBox> &a_advVel, LevelData<FArrayBox> &a_old_vel,
    LevelData<FArrayBox> &U_chi, const LevelData<FArrayBox> &a_viscousSource,
    PatchGodunov &a_patchGodVelocity, Real a_old_time, Real a_dt)
{
  ...
  if (advectionPhysics == nullptr)
  {
    MayDay::Error("AMRLevelMushyLayer::traceAdvectionVel - unable to upcast "
                  "GodunovPhysics to AdvectionPhysics");
  }
  ...
  CornerCopier cornerCopy(a_advVel.disjointBoxLayout(),
                          a_advVel.disjointBoxLayout(), m_problem_domain,
                          a_advVel.ghostVect(), true);
  a_advVel.exchange(cornerCopy); // definitely need this

  CornerCopier cornerCopy2(U_chi.disjointBoxLayout(), U_chi.disjointBoxLayout(),
                           m_problem_domain, U_chi.ghostVect(), true);
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    ...
    {
      LOG_INFO(
          "WARNING - max advection velocity (post tracing, pre-projection) = "
          << maxAdvU);
    }
...
void AMRLevelMushyLayer::correctEdgeCentredVelocity(
    LevelData<FluxBox> &a_advVel, Real a_dt)
{
  ...
  if (m_opt.scaleP_MAC)
  {
    pressureScaleEdgePtrOneGhost = RefCountedPtr<LevelData<FluxBox>>(
        new LevelData<FluxBox>(m_grids, 1, IntVect::Unit));
    fillScalarFace(*pressureScaleEdgePtrOneGhost, half_time, m_pressureScaleVar,
                   true);
    ...
    if (m_level > 0)
    {
      ...
      crsePressureScaleEdgePtr = RefCountedPtr<LevelData<FluxBox>>(
          new LevelData<FluxBox>(mlCrse->m_grids, 1, IntVect::Unit));
      mlCrse->fillScalarFace(*crsePressureScaleEdgePtr, half_time,
                             m_pressureScaleVar, true);

      crsePressureScalePtr = RefCountedPtr<LevelData<FArrayBox>>(
          new LevelData<FArrayBox>(mlCrse->m_grids, 1, IntVect::Unit));
      mlCrse->fillScalars(*crsePressureScalePtr, half_time, m_pressureScaleVar,
                          true);

      crsePressurePtr = RefCountedPtr<LevelData<FArrayBox>>(
          new LevelData<FArrayBox>(mlCrse->m_grids, 1, IntVect::Unit));
      mlCrse->fillScalars(*crsePressurePtr, half_time, m_pressure, true);
    }
  }
  ...
  {
    LOG_DEBUG("Project velocity. Max projections=" << m_opt.maxNumMACProj);

    int projNum = 0;
    Real maxDivU = 10 * m_opt.maxDivUFace; //::computeNorm(*m_scalarNew[ScalarVars::m_divUadv],
                                           //nullptr, 1, m_dx, Interval(0,0));

    while ((maxDivU > m_opt.maxDivUFace && projNum < m_opt.maxNumMACProj) ||
           (m_opt.enforceAnalyticVel &&
            projNum < 1)) // do at least one projection of analytic vel
    {
      projNum++;
      int exitStatus = 0;
      ...
      {
        exitStatus = m_projection.levelMacProject(
            a_advVel, a_dt, crsePressurePtr, pressureScalePtr,
            crsePressureScalePtr, pressureScaleEdgePtrOneGhost,
            crsePressureScaleEdgePtr, false, 1.0);
      }
      ...
      maxDivU = ::computeNorm(*m_scalarNew[ScalarVars::m_divUadv], nullptr, 1,
                              m_dx, Interval(0, 0), 0);
      LOG_INFO("  MAC Projection (#" << projNum << " on level " << m_level
                                     << "), exit status = " << exitStatus
                                     << ", max(div u) = " << maxDivU);
    }
  }
