...
void AMRLevelMushyLayer::computeCCvelocity(LevelData<FArrayBox>& advectionSourceTerm,
                                           Real a_oldTime,
                                           Real a_dt,
                                           bool doFRupdates,
                                           bool a_doProjection,
                                           ...
                                           bool a_MACprojection)
{
  ...
  RefCountedPtr<LevelData<FluxBox> > crsePorosityEdgePtr, porosityEdgePtr;
  RefCountedPtr<LevelData<FArrayBox> > crsePorosityPtr, porosityPtr;
...
//        setVelZero(ccVel, 1e-5);
  ...
  computeUstar(*m_vectorNew[m_UdelU], a_oldTime, a_dt, doFRupdates, a_MACprojection);
  ...
  if (m_scalePwithPorosity)
  {
    ...
    porosityEdgePtr= RefCountedPtr<LevelData<FluxBox> >(new LevelData<FluxBox>(m_grids, 1));

      // Need a ghost cell here for 5 point stencil
      porosityPtr= RefCountedPtr<LevelData<FArrayBox> >(new LevelData<FArrayBox>(m_grids, 1, IntVect::Unit));

      fillScalarFace(*porosityEdgePtr, half_time, m_pressureScaleVar, arithmeticAveraging, true, false, porositySmoothing);
      fillScalars(*porosityPtr, half_time, m_pressureScaleVar, true);
    ...
    if (m_level > 0)
    {
      ...
      if (m_scalePwithPorosity)
      {
        crsePorosityEdgePtr = RefCountedPtr<LevelData<FluxBox> >(new LevelData<FluxBox>(amrMLcrse->m_grids, 1));
        amrMLcrse->fillScalarFace(*crsePorosityEdgePtr, half_time, m_pressureScaleVar, true, porositySmoothing);

        // Need a ghost cell for 5 point stencil
        crsePorosityPtr = RefCountedPtr<LevelData<FArrayBox> >(new LevelData<FArrayBox>(amrMLcrse->m_grids, 1, IntVect::Unit));
        amrMLcrse->fillScalars(*crsePorosityPtr, half_time, m_pressureScaleVar, true);
      }
    }
    ...
    if (m_addSubtractGradP)
    {
      ...
      LevelData<FArrayBox> porosityCentred(m_grids, 1);
      fillScalars(porosityCentred, half_time, m_porosity, true);
      Real old_time = a_oldTime;

      // first get gradPi
      fillPressureSrcTerm(gradP, porosityCentred, old_time-a_dt/2, a_MACprojection);
    ...
    if (m_doProjection && a_doProjection)
    {
      ...
      while(! (sumDivU < initSumDivU*relTol || sumDivU < relTol || i >= maxNumProj))
      {

        setVelZero(*m_vectorNew[uvar], ccVelPorosityLimit);

        m_projection.LevelProject(*m_vectorNew[uvar], crseVelPtr,
                                  new_time, a_dt, porosityPtr, crsePorosityPtr, porosityEdgePtr, crsePorosityEdgePtr,
                                  m_isViscous);
        ...
        sumDivU = computeNorm(*m_scalarNew[m_divU], NULL, -1, m_dx, Interval(0,0), 0);
...
void AMRLevelMushyLayer::fillPressureSrcTerm(LevelData<FArrayBox>& gradP,
                                             LevelData<FArrayBox>& porosityCentred,
                                             Real a_time,
                                             bool a_MACprojection)
{
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    if (m_scalePwithPorosity)
    {
      for (int dir=0;dir<SpaceDim;dir++)
      {
        gradP[dit].mult(porosityCentred[dit], 0, dir);
      }
    }
  }
...
//  LevelData<FArrayBox> lambdaSrc(m_grids, 1);
//  setValLevel(lambdaSrc, 0.0);
//  lambdaSrc.exchange();

  // Make lambda diffusive src
//  computeScalDiffusion(lambdaSrc, m_lambda);
  ...
  int explicitFreestreamCorr = -1;
  ppMain.query("explicitFreestreamCorr", explicitFreestreamCorr);

  AMRLevelMushyLayer* coarseML = getCoarserLevel();

  if (explicitFreestreamCorr >= 0)
  {

    // method 0: use lambda correction
    // method 1: use flux registers

//    int method = 1;
    if (explicitFreestreamCorr == 0 && coarseML)
    {

    // Compute explicit correction on this level
    /*
     * 1) Reflux lambda (will modify coarser level)
     * 2) Compute correction term (on this level only)
     * 3) Reset lambda (on both this and coarser levels)
     */
    int numLevels =  getFinestLevel()+1;
    ...
    // Compute correction from coarser level
    coarseML->doExplicitReflux(m_lambda);
    (coarseML->m_projection).computeVDCorrection(amrLambda, amrPorosityFace, m_time, m_dt);

    // Apply correction on this level
    m_projection.applyFreestreamCorrection(m_advVel);

    /// Let's also write out the correction
    setValLevel(m_advVelExplicitCorrection, 0.0);
    m_projection.applyFreestreamCorrection(m_advVelExplicitCorrection);
    EdgeToCell(m_advVelExplicitCorrection, *m_vectorNew[m_advVelCorr]);

    }
    else if (explicitFreestreamCorr == 1)
    {
// Old code (done incorrectly)

    // Increment flux registers instead of advecting lambda
    Real fluxMult = m_dt;
    ...
    if (m_advVelFR)
    {
      for (DataIterator dit = m_advVel.dataIterator(); dit.ok(); ++dit)
      {
        for (int dir=0; dir < SpaceDim; dir++)
        {
          m_advVelFR->incrementCoarse(
              m_advVel[dit][dir], fluxMult, dit(), Interval(0,0),
              Interval(0,0), dir);
        }
      }
    }
...
//    AMRLevelMushyLayer* crseML = getCoarserLevel();
    ...
    if (coarseML)
    {
      // Increment FR on coarser level
      RefCountedPtr<LevelFluxRegister> crseFR = coarseML->m_advVelFR;
      for (DataIterator dit = m_advVel.dataIterator(); dit.ok(); ++dit)
      {
        for (int dir=0; dir < SpaceDim; dir++)
        {
          crseFR->incrementFine(
              m_advVel[dit][dir], fluxMult, dit(), Interval(0,0),
              Interval(0,0), dir);
        }
      }

      // Check if we're doing the last subcycled step
      // i.e. this time = crse time

      if (abs(m_time-coarseML->m_time) < TIME_EPS)
      {

        LevelData<FArrayBox> coarseReflux(coarseML->m_grids, SpaceDim, m_advVel.ghostVect());
        LevelData<FArrayBox> fineReflux(m_grids, SpaceDim, m_advVel.ghostVect());

        setValLevel(coarseReflux, 0.0);
        setValLevel(fineReflux, 0.0);

        Real refluxScale = -1/coarseML->m_dx;
        refluxScale = -1/coarseML->m_dt;
        crseFR->reflux(coarseReflux, refluxScale);

        m_quadCFInterpVector.coarseFineInterp(fineReflux, coarseReflux);
        CellToEdge(fineReflux, m_advVelExplicitCorrection);
...
//            m_advVelExplicitCorrection[dit][dir].divide(m_dt);
          }

          Box b = m_advVel[dit].box();
          m_advVel[dit].minus(m_advVelExplicitCorrection[dit], b, 0, 0, 1);
        }
    ...
    // Reset lambda and advect again with new velocity
    m_scalarOld[m_lambda]->copyTo(Interval(0,0), *m_scalarNew[m_lambda], Interval(0,0));
    if (coarseML)
    {
    coarseML->m_scalarOld[m_lambda]->copyTo(Interval(0,0), *(coarseML->m_scalarNew[m_lambda]), Interval(0,0));
    }

    bool use_lambda_porosity = false;
    ppMain.query("use_lambda_porosity", use_lambda_porosity);

    if (use_lambda_porosity)
    {
      advectScalar(m_lambda, m_lambda_porosity, m_advVel, doFRupdates);
    }
    else
    {
      advectScalar(m_lambda, m_lambda, m_advVel, doFRupdates);
    }

    // Remove flux from FRs before we do the proper update
    updateScalarFluxRegister(m_lambda, lambdaFlux, -m_dt);

  } // end if doing explicit  lambda correction
...
//  fillVectorField(U_scaled, oldTime, m_fluidVel, true);
  ...
  fillPressureSrcTerm(P_src, porosity_centred, oldTime-m_dt/2, a_MACprojection);
...
void AMRLevelMushyLayer::computeUstar(LevelData<FArrayBox>& a_UdelU, Real a_oldTime, Real a_dt, bool doFRupdates,
                                      bool a_MACprojection)
{
  ...
  computeUstarSrc(src, a_UdelU, src_time, a_MACprojection);
...
void AMRLevelMushyLayer::calculateTimeIndAdvectionVel(Real time, LevelData<FluxBox>& a_advVel)
{
  ...
  VelBCHolder velBC(m_physBCPtr->uStarFuncBC(m_viscousBCs));
  VelBCHolder velBCExtrap(m_physBCPtr->velExtrapBC(m_viscousBCs));
  ...
  //      LevelData<FArrayBox> pressure(m_grids, 1, IntVect::Unit);
  fillScalarFace(permeability_face, time, m_permeability,        true, false);
  fillScalarFace(T_face,            time, m_temperature,         true, false); // second true - do quadinterp.
  ...
  else
  {
    ...
    for (dit.reset(); dit.ok(); ++dit)
    {
      ...
      for (int idir=0; idir<SpaceDim; idir++)
      {
        ...
        // Try use previous pressure to get this right?
        //        a_advVel[dit][idir].minus(gradPhi[dit][idir]);
        //        alreadyHasPressure = true;

        // was this the issue
        a_advVel[dit][idir].mult(permeability_face[dit][idir]);

        //          (*permeabilityPtr)[dit].mult(-1.0);
        //          (*permeabilityEdgePtr)[dit][0].mult(-1.0);
        //          (*permeabilityEdgePtr)[dit][1].mult(-1.0);
      }
  ...
  //  EdgeToCell(a_advVel, *m_vectorNew[m_Ustar]);

  // For debugging
  //  Divergence::levelDivergenceMAC(*m_scalarNew[m_divU], a_advVel, m_dx);
  ...
  // Don't need to do this! Should be correct after MAC projection
  //  edgeVelBC.applyBCs(a_advVel, m_grids,
  //                     m_problem_domain, m_dx,
  //                     false); // inhomogeneous
  ...
  bool enforceAnalyticVel = (m_parameters.physicalProblem == MushyLayerParams::m_soluteFluxTest
      //      || m_parameters.physicalProblem == MushyLayerParams::m_HRL
  );
...
void AMRLevelMushyLayer::predictVelocities(LevelData<FArrayBox>& a_uDelU,
                                           ...
                                           LevelData<FArrayBox>& viscousSource,
                                           Real old_time,
                                           Real a_dt,
                                           bool doFRupdates)
{
  ...
  { //this is to allow this fluxbox to go out of scope
    ...
    // Get the true porosity correction (scaled with chi if appropriate)
    if (m_scalePwithPorosity)
    {
      for (DataIterator dit=gradPhi.dataIterator(); dit.ok(); ++dit)
      {
        for (int dir=0; dir<SpaceDim; dir++)
        {
          gradPhi[dit].mult(porosity[dit], porosity[dit].box(), 0, dir);
        }
      }
    }
    ...
    viscousSource.exchange();
    grad_eLambda.exchange();
    gradPhi.exchange();

    computePredictedVelocities(advectedField, traceVel, advectionVelocity,
                               UtoAdvect_old, viscousSource, m_patchGodVelocity,
                               grad_eLambda, gradPhi, porosity, old_time, a_dt,
                               legacyCompute);
...
//  setVelZero(advectedField, advVelChiLimit);
  ...
  for (dit.reset(); dit.ok(); ++dit)
  {
    ...
    for (int dir = 0; dir < SpaceDim; dir++)
    {
      FArrayBox& this_uEdge = advectedField[dit][dir];
    ...
    if (doFRupdates)
    {
        ...
        for (int dir = 0; dir < SpaceDim; dir++)
        {
          ...
          if (s_reflux_normal_momentum)
          {
            m_vectorFluxRegisters[reflux_var]->incrementCoarse(
                advectedField[dit][dir], fluxMult, dit(), velComps,
                velComps, dir);
          } else {
            ...
            for (int velComp = 0; velComp < SpaceDim; velComp++)
            {
              if (velComp != dir)
              {
                ...
                m_vectorFluxRegisters[reflux_var]->incrementCoarse(
                    advectedField[dit][dir], fluxMult, dit(),
                    velInt, velInt, dir);
      ...
      if (m_level > 0)
      {
        ...
        for (int dir = 0; dir < SpaceDim; dir++)
        {
          if (s_reflux_normal_momentum)
          {
            crseFR.incrementFine(advectedField[dit][dir], fluxMult, dit(),
            ...
            for (int velComp = 0; velComp < SpaceDim; velComp++)
            {
              if (velComp != dir)
              {
                ...
                crseFR.incrementFine(advectedField[dit][dir], fluxMult,
                                           ...
                                           PatchGodunov& a_patchGodVelocity, Real a_old_time, Real a_dt)
{
  ...
  {
    MayDay::Error(
        "AMRLevelMushyLayer::computeAdvectionVelocities - unable to upcast GodunovPhysics to AdvectionPhysics");
  }
...
void AMRLevelMushyLayer::computePredictedVelocities(
    ...
    LevelData<FArrayBox>& a_viscousSource, PatchGodunov& a_patchGodVelocity,
                                       ...
                                       LevelData<FArrayBox>& a_vel, const LevelData<FArrayBox>* a_crseVelPtr)
{
  ...
  // original velocity
  LevelData<FArrayBox> a_vel_copy(m_grids, SpaceDim, IntVect::Unit);
  DataIterator dit(m_grids);
  for (dit.begin(); dit.ok(); ++dit)
  {
...
//  m_projection.gradPiBCs(pressure, true);
  fillPressureSrcTerm(pressure, porosity, src_time-m_dt/2,
