 ...
 *      Author: parkinsonjl
 *
 *      This structure contains all the options for the mushy layer code which
 * shouldn't change during a simulation
 */

#ifndef SRC_MUSHYLAYEROPT_H_
#define SRC_MUSHYLAYEROPT_H_

// Comment below ensures that enums doxygen generates documentation for enums
...
enum RefinementMethod
{
  ...
  /**
   * Tag where \f$ \Delta x |\nabla \chi| ( 1 - \textrm{min}(w (\mathscr{C} -
   * \Theta), 0)   ) > r \f$ where \f$ r \f$ is the refinement threshold, \f$ w
   * \f$ is the vertical velocity,
   */
...
enum TaggingMethod
{
  /// 0 - Tag where the undivided gradient of some field is bigger than some
  /// value
  UndividedGradient = 0,
  ...
  /// 3 - Tag where the value of some field multiplied by -1 is larger than some
  /// criteria.
  /**
   * Alternatively, can think of this as when some field is smaller than some
   * criteria.
   */
  CompareLargerThanNegative
};

/// Methods for doing implicit refluxing - i.e. what sort of equation to we
...
{
  /// solve \f$ \frac{\partial (\mathbf{u}/\chi)}{\partial t} + (u/porosity)
...
enum PorosityFunctions
{
  ...
  /// Porosity looks like a gaussian, \f$ \chi =  0.1 + 0.9 exp(-100 (x-0.5)^2)
  /// \f$
  gaussian,

  /// Gaussian with some sinuosoidal variation, \f$ \chi =
  /// (exp(-(x-0.5)^2))*(0.5 + 0.1 sin(2 \pi y)) \f$
  gaussianSinusoidal,

  /// Porosity = 0.1
  constantSmall,

  /// Porosity varies with the y-direction cubed, \f$ \chi = 0.01 + (y-0.01)^3
  /// \f$
  cubic,
...
enum VectorVars
{
  ...
  /// Unprojected face centred fluid velocity, as used for advection, which has
  /// been averaged to cell centres
  /**
   * \f$ {Av}^{F \to C} \mathbf{U}_f \f$
   */
  m_advUstar,

  /// Face centred fluid velocity, as used for advection, which has been
  /// averaged to cell centres
  /**
...
enum ScalarVars
{
  ...
  /// Auxillary field lambda \f$ \lambda \f$ for computing the freestream
  /// correction
  m_lambda,
  ...
  /**
   * \f$ - \nabla \cdot \left( \mathbf{U} \Theta_l \right) - V \frac{\partial
   * \Theta}{\partial z} \f$
   */
  m_saltEqnSrcGodunov,
  ...
  /**
   * \f$ - \nabla \cdot \left( \mathbf{U} \theta \right) - V \frac{\partial
   * H}{\partial z} \f$
   */
  m_enthalpySrc,
  ...
  /// Rate of change of bulk concentration with time \f$ \frac{\partial
  /// \Theta}{\partial t} \f$
  m_dSdt,
  ...
  /// Vertical component of the solute flux \f$ \mathbf{F}_s \cdot \mathbf{z}
  /// \f$
  m_verticalFlux,
...
// TODO: collate more options in here rather than having lots of parmparses in
// the main code, so it's obvious what all the options are
/// Contains most of the options for running the code, and how to handle the
/// equations
struct MushyLayerOptions
{
  ...
  /// Maximum value by which the solution may have changed for us to have
  /// reached steady state
  Real steadyStateCondition;

  /// Turn on to ignore changes in velocity when deciding if we've reached
  /// steady state
  bool ignoreVelocitySteadyState;

  /// Turn on to ignore changes in bulk concentration when deciding if we've
  /// reached steady state
  ...
  /// Force the code to compute a CFL condition based on the size of
  /// \f$\mathbf{U}/\chi\f$, rather than \f$ \mathbf{U} \f$
  bool forceUseUChiForCFL;

  /// Minimum time to run simulations for (even if the steady state condition
  /// has been met)
  Real min_time;
  ...
  /// Multiply the dt computed during initialisation procedures by this factor
  /// before using it
  /**
   * Useful for using a smaller dt initially for stability
   * TODO: can probably remove this, it's just a duplicate of
   * initial_dt_multiplier really
   */
  Real init_dt_scale;
  ...
  /**
   * When velocities are changing rapidly due to a large acceleration, the
   * timestep computed based on the current velocities may not be a appropriate.
   * Instead, we should estimate the new velocities given the acceleration:
   *
   * \f$ \Delta t = \sqrt{\sigma  \Delta x / a} \f$,
   *
   * where \f$ \sigma \f$ is the cfl number and \f$ a\f$ is the maximum
   * acceleration.
   *
   */
  Real accelCFL;

  /// Turn on to print some diagnostics about the dt computed according to
  /// acceleration considerations
  bool printAccelDt;

  /// Use the timesteps computed according to the cfl condition applied to
  /// accelerations
  bool useAccelDt;

  /// For the first timestep, use dt computed according to the cfl condition
  /// applied to accelerations
  bool useInitAccelDt;
  ...
  /// Turn on to use fortran routines for regularising the solution on cell
  /// faces (i.e. ensuring porosity is not 0)
  /**
   * Turned off by default as doesn't work yet.
   */
  bool useFortranRegularisationFace;

  /// Turn on using fortran routines for regularising the solution on cell
  /// centres (i.e. ensuring porosity is not 0)
  ...
  /// Whether to use previous estimates of the pressure to compute the initial
  /// cell centred \f$ \mathbf{U}^* \f$
  /**
   ...
   * temperature to cell faces. As the temperature also obeys diffuses, we can
   * add a diffusive source term to estimate the upwinded temperatures more
   * accurately. Similarly for other advected and diffused quantities.
   */
  bool doDiffusionSrc;
  ...
  /// When computing the advection velocity during multiple initialisation
  /// steps, grow MushyLayerOptions::initAdvVelCentering by this factor with
  /// each iteration
  ...
  /**
   * Turned off by default, as this won't always work with some of the restart
   * files we generate (e.g. after refinement/adding new cells). This is only
   * really useful for when we want to do post processing without recomputing
   * velocity fields.
   */
  bool load_advVel;

  /// Whether to do 1st order (set to 1) or 2nd order (set to 2) interpolation
  /// at coarse-fine boundaries
  int CFinterpOrder_advection;

  /// Whether we want to try and do some dodgy stuff to make the enthalpy-bulk
  /// concentration nonlinear solver quicker
  /**
   * Turned off by default, use with extreme caution.
   */
  bool nonlinearHCOpSuperOptimised;

  /// Whether to apply BCs to temperature, porosity etc. explicitly during
  /// multigrid solves
  /**
  ...
  /**
   * If true, we then only solve for a small extra pressure correction which is
   * usually a lot quicker
   */
  bool useIncrementalPressure;

  /// Whether or not to solve for an incremental pressure change on fine levels,
  /// rather than the full pressure
  /**
  ...
  /// Whether or not to do synchronisation operations over multiple levels have
  /// reached the same time
  bool doSyncOperations;

  /// Whether or not to enforce an analytic solution at the start of the
  /// simulation
  ...
  /// Try and make sure the maximum divergence of the face centred velocity is
  /// less than this
  Real maxDivUFace;

  /// Whether or not the MAC (face-centred) projection should solve for a
  /// pressure that is scaled by the porosity or permeability
  /**
   * If scaled, we solve something like \f$ \nabla \cdot \chi \nabla \phi =
   * \nabla \cdot \mathbf{U}^* \f$, else we'll solve \f$ \nabla^2 \phi = \nabla
   * \cdot \mathbf{U}^* \f$.
   */
  bool scaleP_MAC;

  /// Whether or not the CC (cell-centred) projection should solve for a
  /// pressure that is scaled by the porosity
  /**
   * If scaled, we solve something like \f$ \nabla \cdot \chi \nabla \pi =
   * \nabla \cdot \mathbf{U}^* \f$, else we'll solve \f$ \nabla^2 \pi = \nabla
   * \cdot \mathbf{U}^* \f$.
  ...
  /// Use the cell centred pressure \f$ \pi \f$ as the boundary condition when
  /// solving for the face centred pressure \f$ \phi \f$
  /**
   * Default is true.
   */
  ...
  /// If > 0, use Darcy's equation to compute velocities in cells with porosity
  ...
  /// Porosity below which we ensure that the src term for computing advection
  /// velocities is 0
  Real advVelsrcChiLimit;

  /// Porosity below which we ensure that the \f$ \mathbf{U} \cdot \nabla
  /// \mathbf{U} \f$ part of the cell-centred source term is 0
  Real uDelU_porosityLimit;

  /// Porosity below which we ensure that velocities are zero when computing \f$
  /// \mathbf{U} \cdot \nabla \mathbf{U} \f$
  Real advVelChiLimit;

  /// Use advection velocity from previous timestep for tracing advection
  /// velocities
  /**
   * Default is false, in which case we average the cell centred velocities from
   * the previous timestep to cell faces and use these (as they are calculated
   * at a later time, they should be more accurate).
   */
  bool useOldAdvVel;
  ...
  /// Whether or not to initialise the velocity to an analytically specified
  /// value
  ...
  /// Accuracy for boundary conditions applied to laplacian(velocity) in the
  /// advection velocity source term
  int lapVelBCOrder;

  /// Where to compute the CC velocity source term (default = 0.5 i.e. halfway
  /// through the timestep)
  ...
  /// Whether or not to include the viscous term in the advection velocity
  /// source
  ...
  /// in the cell centred velocity source term
  bool CCAdvSrc;

  /// Whether or not to include buoyancy in the cell centred velocity source
  /// term
  bool CCBuoyancySrc;

  /// Whether or not to include pressure in the cell centred velocity source
  /// term
  /**
  ...
  /// Turn to make sure we use MushyLayerOptions::CCPressureSrc to decide
  /// whether or not to include pressure
  /**
   * Otherwise, we will use AMRLevelMushyLayer::m_usePrevPressureForUStar.
   * This is turned on by default if MushyLayerOptions::CCPressureSrc is defined
   * through the inputs file
   */
  ...
  /**
   * If you don't do this, you won't be able to calculate the freestream
   * preservation correction.
   */
  bool reflux_lambda;

  /// Whether or not to average down from fine levels to coarser levels after
  /// refluxing
  /**
   * As the solution will have changed following refluxing, doing this ensures
  ...
  /**
   * This might have to change depending on the operator being used, so left as
   * an option.
   */
  Real refluxBetaSign;
  ...
  /**
   * This might need to be changed depending on the operator being used, so left
   * as an option.
   */
  Real refluxCorrSign;

  /// Solver tolerance for the viscous solver, which we use for smoothing scalar
  /// fields and doing momentum reflux
  ...
  /// If residual norm is less than this absolute value, solver decides we've
  /// converged
  Real AMRMultigridNormThresh;

  /// Multigrid for implicit cell centred \f$ \mathbf{U}^* \f$ solve - num
  /// smoothing steps
  int velMGNumSmooth;
  ...
  /// Multigrid for the coupled enthalpy-bulk concentration solve: number of
  /// smooths on each up sweep
  ...
  /**
   * We use this here as it treats the nonlinearity more sensibly than linear
   * solvers like BiCGStab
   */
  ...
  /// Whether to use artificial viscosity for enthalpy-bulk concentration
  /// advection
  bool HCUseArtVisc;

  /// Value of artificial viscosity to use for enthalpy-bulk concentration
  /// advection
  Real HCArtVisc;
  ...
  /// Whether to use a higher order flux limiter for enthalpy-bulk concentration
  /// advection
  ...
  /// If this is > 0, then when time > fixed_grid_time we will switch to the
  /// grid specified by 'main.regrid_gridfile'
  ...
  /// Turn on to only tags cells where other refinement criteria are met and
  /// porosity \f$\chi < 1 \f$
  ...
  /// Whether we should tag cells where the fluid velocity (magnitude) is
  /// greater than MushyLayerOptions::vel_thresh
  bool tag_velocity;

  /// New option for a refinement method based on a composite criteria
  bool compositeChannelTagging;

  /// If MushyLayerOptions::tag_velocity is true, specify the velocity magnitude
  /// criteria here
  Real vel_thresh;

  /// Refine mushy regions on level 1, the refine around plumes according to
  /// some empirical criteria
  /**
   * Specifically, identify plumes as regions where either
   * a) the downwards velocity is greater than
   * MushyLayerOptions::plumeVelThreshold, and b) the bulk concentration is
   * greater than MushyLayerOptions::plumeSalinityThreshold
   */
  bool tag_plume_mush;
  ...
  /// If less than 1, use this limit to identify cells with
  /// taggingMarginalPorosityLimit < porosity < 1.0
  /**
   * These cells are then not considered part of the mushy layer for tagging
   * purposes, but also not liquid.
   */
  Real taggingMarginalPorosityLimit;

  /// Tag cells in the centre of the domain to make a box of this size. Leave as
  /// 0 (default) to do nothing
  int tagCenterBoxSize;

  /// If MushyLayerOptions::tagCenterBoxSize > 0, then regrid once time >
  /// tagCenterBoxRegridTime
  Real tagCenterBoxRegridTime;

  /// For testing: if turned on, refined levels will only exist for one regrid
  /// cycle before being removed
  /**
   * This let's us test levels being created then destroyed in a controlled
   * manner
   */
  bool testRegridCoarsening;

  /// Whether to do higher order interpolationg for scalars to fill new cells
  /// following regridding
  bool scalarHOinterp;

  /// Whether to do higher order interpolation for vectors to fill new cells
  /// following regridding
  bool vectorHOinterp;
  ...
  /// Factor by which to scale dt for re-initialising pressure following
  /// regridding
  ...
  /// Whether or not to advect lambda after regridding, in order to recompute
  /// the freestream correction
  ...
  /// Option to scale \f$ \eta \f$ before recomputing the freestream correction
  /// after regridding
  /**
   * Default value = 1.0.
   */
  ...
  /// Whether to reflux lambda before recomputing freestream correction after
  /// regridding
  ...
  /// Coefficient to determine how aggressively to do smoothing (if
  /// MushyLayerOptions::do_postRegrid_smoothing = true)
  ...
  /// When initialising pressure via multiple iterations, decide whether or not
  /// to reset the other fields between iterations
  bool initResetStates;

  //  Real skipTrickySourceTerm;

  /// Advect both enthalpy and bulk concentration at the same time (slightly
  /// more efficient)
  bool allowMulticompAdvection;

  //  Real smoothingCoeff;

  /// Whether or not to compute the volume discrepancy correction at the start
  /// of a simulation
  ...
  /// Whether or not we should do advection and diffusion of scalars (enthalpy,
  /// bulk concentration)
  ...
  /// Wavenumber of sinusoidal perturbations (default = 0.0, i.e. just a uniform
  /// value)
  Real perturbationWavenumber;

  /// If true, sinusoidal perturbations take the form of a sin wave. Otherwise a
  /// cosine wave.
  /**
  ...
  /**
   * If < 0, use some functional form for the porosity as defined by
   * MushyLayerOptions::porosityFunction
   */
  ...
  /// Functional form to use for the porosity if using a fixed porosity (rather
  /// than a porosity computed from the solution).
  PorosityFunctions porosityFunction;

  /// When restarting, if MushyLayerOptions::restartPerturbation > 0.0 then add
  /// a perturbation which if the sum of waves with wavenumbers from
  /// 1->maxRestartWavenumber
  ...
  /// Whether to initialise with a linear enthalpy gradient in the vertical
  /// direction
  bool linearGradient;

  /// Approximate depth of mushy layer to create as part of an initial condition
  /// approximating some sea ice
  Real mushHeight;

  /// Different options for initialising data that looks a bit like year old sea
  /// ice
  /**
  ...
  /// Previous reference salinity for nondimensionalisation (if restarting and
  /// different from the new value)
  Real prevRefSalinity;

  /// Previous reference temperature for nondimensionalisation (if restarting
  /// and different from the new value)
  Real prevRefTemp;

  /// Facotr by which to reduce dt after restarting: \f$ dt = dt /
  /// dtReductionFactor \f$
  ...
  /// For a fixed porosity which varies linearly from some constant value inside
  /// the domain to the edge, this is the radius of the inner region
  Real fixedPorosityFractionalInnerRadius;

  /// If using a time dependent enforced porosity, stop changing it after this
  /// time has passed
  ...
  /**
   * Should really be FAS as we're solving nonlinear problems, but leaving the
   * option here to change back to linear multigrid.
   */
  MGmethod MGtype;

  /// Specify how to treat the \f$ \mathbf{U} \cdot \nabla \left(
  /// \mathbf{U}/\chi \right) \f$ term
