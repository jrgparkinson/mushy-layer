  ...
  for (dit.begin(); dit.ok(); ++dit) {
    ...
    //		This_will_not_compile!
        ...
        (CHF_FRA(a_lhs[dit]), CHF_CONST_FRA(phi[dit]),
         ...
         CHF_CONST_FRA((*m_aCoef)[dit]), CHF_CONST_FRA((*m_cCoef)[dit]),
...
void DarcyBrinkmanOp::applyOpNoBoundary(LevelData<FArrayBox> &a_lhs,
  ...
  for (dit.begin(); dit.ok(); ++dit) {
    ...
    //		This_will_not_compile!
#endif
        (CHF_FRA(a_lhs[dit]), CHF_CONST_FRA(phi[dit]), CHF_CONST_REAL(m_alpha),
         CHF_CONST_FRA((*m_aCoef)[dit]), CHF_CONST_FRA((*m_cCoef)[dit]),
         CHF_CONST_REAL(m_beta),
#if CH_SPACEDIM >= 1
         CHF_CONST_FRA(thisBCoef[0]),
  ...
  for (DataIterator dit = a_phiFine.dataIterator(); dit.ok(); ++dit) {
    ...
    //		This_will_not_compile!
        ...
        (CHF_FRA_SHIFT(res, civ), CHF_CONST_FRA_SHIFT(phi, iv),
         ...
         CHF_CONST_FRA_SHIFT(thisACoef, iv), CHF_CONST_FRA_SHIFT(thisCCoef, iv),
         CHF_CONST_REAL(m_beta),
#if CH_SPACEDIM >= 1
...
void DarcyBrinkmanOp::setAlphaAndBeta(const Real &a_alpha, const Real &a_beta) {
  m_alpha = a_alpha;
  m_beta = a_beta;
  ...
  if (m_lambdaNeedsResetting) {
    ...
    for (DataIterator dit = m_lambda.dataIterator(); dit.ok(); ++dit) {
      ...
      for (int dir = 0; dir < SpaceDim; dir++) {
        FORT_SUMFACES3(CHF_FRA(lambdaFab), CHF_CONST_REAL(m_beta),
  ...
  for (ditf.reset(); ditf.ok(); ++ditf) {
    ...
    for (int idir = 0; idir < SpaceDim; idir++) {
      ...
      for (sit.begin(); sit.ok(); sit.next()) {
        ...
        FArrayBox fineflux;
        getFlux(fineflux, phifab, fineBCoef, facebox, idir, m_refToFiner);
...
void DarcyBrinkmanOp::getFlux(FArrayBox &a_flux, const FArrayBox &a_data,
...
// Factory
DarcyBrinkmanOpFactory::DarcyBrinkmanOpFactory() { setDefaultValues(); }

//-----------------------------------------------------------------------
//  AMR Factory define function
void DarcyBrinkmanOpFactory::define(
...
void DarcyBrinkmanOpFactory::define(const ProblemDomain &a_coarseDomain,
                                    ...
                                    const Real &a_coarsedx, BCHolder a_bc,
...
DarcyBrinkmanOpFactory::MGnewOp(const ProblemDomain &a_indexSpace, int a_depth,
                                bool a_homoOnly) {
  ...
  if (coarsening > 1 &&
      !m_boxes[ref].coarsenable(coarsening * DarcyBrinkmanOp::s_maxCoarse)) {
    return nullptr;
  }
...
DarcyBrinkmanOpFactory::AMRnewOp(const ProblemDomain &a_indexSpace) {
...
//-----------------------------------------------------------------------
void DarcyBrinkmanOp::finerOperatorChanged(
    const MGLevelOp<LevelData<FArrayBox>> &a_operator, int a_coarseningFactor) {
  const DarcyBrinkmanOp &op = dynamic_cast<const DarcyBrinkmanOp &>(a_operator);
