...
void updateEnthalpyVariables(
    LevelData<FArrayBox> &HC, LevelData<FArrayBox> &temperature,
    LevelData<FArrayBox> &compositionLiquid,
    ...
    LevelData<FArrayBox> &enthalpyEutectic, const MushyLayerParams &a_params)
{
  ...
  updateEnthalpyVariables(enthalpy, composition, temperature, compositionLiquid,
                          compositionSolid, porosity, enthalpySolid,
                          enthalpyLiquid, enthalpyEutectic, a_params);
}

void updateEnthalpyVariables(LevelData<FArrayBox> &HC,
                             LevelData<FArrayBox> &theta,
                             LevelData<FArrayBox> &compositionLiquid,
                             LevelData<FArrayBox> &compositionSolid,
                             LevelData<FArrayBox> &porosity,
                             const MushyLayerParams &a_params)
{
  ...
  updateEnthalpyVariables(enthalpy, composition, theta, compositionLiquid,
                          compositionSolid, porosity, a_params);
}

void updateEnthalpyVariables(LevelData<FArrayBox> &enthalpy,
                             LevelData<FArrayBox> &composition,
                             LevelData<FArrayBox> &theta,
                             LevelData<FArrayBox> &compositionLiquid,
                             LevelData<FArrayBox> &compositionSolid,
                             LevelData<FArrayBox> &porosity,
...
void updateEnthalpyVariables(
    LevelData<FArrayBox> &enthalpy, LevelData<FArrayBox> &composition,
    ...
    LevelData<FArrayBox> &enthalpyEutectic, const MushyLayerParams &a_params)
{
  ...
  for (DataIterator dit = temperature.dataIterator(); dit.ok(); ++dit)
  {
    ...
    FORT_CALCULATE_BOUNDING_ENERGY(
        CHF_CONST_FRA(HC), CHF_FRA(Hs), CHF_FRA(He), CHF_FRA(Hl),
        CHF_BOX(region), CHF_CONST_REAL(a_params.compositionRatio),
        CHF_CONST_REAL(a_params.waterDistributionCoeff),
        CHF_CONST_REAL(a_params.specificHeatRatio),
        CHF_CONST_REAL(a_params.stefan), CHF_CONST_REAL(a_params.thetaEutectic),
        CHF_CONST_REAL(a_params.ThetaEutectic));

    // Now we calculate porosity, temperature and liquid/solid composition at
    // each grid point based on what enthalpy regime we are in
    FORT_CALCULATEPOROSITY(CHF_FRA(chi), CHF_CONST_FRA(H), CHF_CONST_FRA(C),
                           CHF_CONST_FRA(Hs), CHF_CONST_FRA(He),
                           CHF_CONST_FRA(Hl), CHF_BOX(region),
    ...
    FORT_CALCULATECL(CHF_FRA(Cl), CHF_CONST_FRA(H), CHF_CONST_FRA(C),
                     CHF_CONST_FRA(Hs), CHF_CONST_FRA(He), CHF_CONST_FRA(Hl),
                     CHF_BOX(region), CHF_CONST_REAL(a_params.compositionRatio),
                     CHF_CONST_REAL(a_params.waterDistributionCoeff),
                     CHF_CONST_REAL(a_params.specificHeatRatio),
                     CHF_CONST_REAL(a_params.stefan),
    ...
    FORT_CALCULATECS(CHF_FRA(Cs), CHF_CONST_FRA(H), CHF_CONST_FRA(C),
                     CHF_CONST_FRA(Hs), CHF_CONST_FRA(He), CHF_CONST_FRA(Hl),
                     CHF_BOX(region), CHF_CONST_REAL(a_params.compositionRatio),
                     CHF_CONST_REAL(a_params.waterDistributionCoeff),
                     CHF_CONST_REAL(a_params.specificHeatRatio),
    ...
    FORT_CALCULATET(CHF_FRA(T), CHF_CONST_FRA(H), CHF_CONST_FRA(C),
                    CHF_CONST_FRA(Hs), CHF_CONST_FRA(He), CHF_CONST_FRA(Hl),
                    CHF_BOX(region), CHF_CONST_REAL(a_params.compositionRatio),
                    CHF_CONST_REAL(a_params.waterDistributionCoeff),
                    CHF_CONST_REAL(a_params.specificHeatRatio),
...
void computeEnthalpyVars(const Real H, const Real C, Real &porosity,
                         Real &theta, Real &C_l, Real &C_s, const Real H_s,
                         const Real H_l, const Real H_e,
                         const Real specificHeatRatio, const Real stefan,
                         Real const compositionRatio,
                         ...
                         const Real thetaEutectic, const Real ThetaEutectic)
{
  ...
  {
    porosity = (H - thetaEutectic * specificHeatRatio) /
               (stefan + thetaEutectic * (1 - specificHeatRatio));
    C_l = ThetaEutectic;
    C_s = C / (1 - porosity);
    theta = thetaEutectic;
  }
  else if (H > H_e && H < H_l)
  {
    ...
    C_l = (C + compositionRatio * (1 - porosity)) /
          (porosity + waterDistributionCoeff * (1 - porosity));
    C_s = (waterDistributionCoeff * C - compositionRatio * porosity) /
          (porosity + waterDistributionCoeff * (1 - porosity));
    theta = -C_l;
  }
  else
...
Real computeTemperature(Real H, Real C, Real compositionRatio,
                        Real specificHeatRatio, Real stefan,
                        Real waterDistributionCoeff, Real heatCapacityRatio,
                        Real thetaEutectic, Real ThetaEutectic)
{
  Real H_e, H_s, H_l, theta;

  ::computeBoundingEnergy(H, C, H_s, H_l, H_e, heatCapacityRatio, stefan,
  ...
  else if (H > H_e && H < H_l)
  {
    Real porosity =
        computePorosityMushyLayer(H, C, compositionRatio, specificHeatRatio,
                                  stefan, waterDistributionCoeff);
    theta = -(C + compositionRatio * (1 - porosity)) /
            (porosity + waterDistributionCoeff * (1 - porosity));
  }
  else
  {
...
Real computePorosity(Real H, Real C, Real compositionRatio,
                     Real specificHeatRatio, Real stefan,
                     Real waterDistributionCoeff, Real heatCapacityRatio,
                     Real thetaEutectic, Real ThetaEutectic)
{
  ...
  ::computeBoundingEnergy(H_e, C, H_s, H_l, H_e, heatCapacityRatio, stefan,
                          compositionRatio, waterDistributionCoeff,
                          thetaEutectic, ThetaEutectic);
...
Real computePorosityMushyLayer(Real H, Real a_C, Real compositionRatio,
                               Real specificHeatRatio, Real stefan,
                               Real waterDistributionCoeff)
{
  ...
  {
    Real a = compositionRatio * (specificHeatRatio - 1) +
             stefan * (waterDistributionCoeff - 1);
    Real b = compositionRatio * (1 - 2 * specificHeatRatio) +
             H * (1 - waterDistributionCoeff) - a_C * (specificHeatRatio - 1) -
             waterDistributionCoeff * stefan;
...
void computeBoundingEnergy(const Real H, const Real C, Real &H_s, Real &H_l,
                           Real &H_e, const Real heatCapacityRatio,
                           const Real stefan, const Real compositionRatio,
                           const Real waterDistributionCoeff,
                           const Real thetaEutectic, const Real ThetaEutectic)
{
  Real zero = 0.0;
  H_s = heatCapacityRatio *
        (thetaEutectic +
         max(zero, (-C - compositionRatio) / waterDistributionCoeff));

  Real chi_e = (compositionRatio + C) / (ThetaEutectic + compositionRatio);
  H_e = chi_e * (stefan + thetaEutectic * (1 - heatCapacityRatio)) +
        heatCapacityRatio * thetaEutectic;
