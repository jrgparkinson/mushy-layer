                            ...
                            MushyLayerParams a_parameters)
{
  ...
  for (DataIterator dit = enthalpyAnalytic.dataIterator(); dit.ok(); ++dit)
  {
    ...
    for (BoxIterator bit(b); bit.ok(); ++bit)
    {
      ...
      S(iv) = a_parameters.bcValBulkConcentrationLo[1] +
              (0.2) * exp(-pow(100 * (x - (a_domainWidth / 2)), 2)) * S_vert;
    }
  }
}
...
void analyticSolnSolidificationNoFlow(
    ...
    Real a_dx, MushyLayerParams a_parameters)
{
  ...
  numPoints = numPoints *
              4; // Use an even finer grid to make interpolation more accurate
  ...
  Real zEutectic = a_domainLength; // Position of the eutectic. For now fix this
  ...
  if (thetaTop < a_parameters.thetaEutectic)
  {
    ...
    zEutectic = (HBottom - a_parameters.thetaEutectic) / (HBottom - HTop) *
                a_domainLength;
  }
  ...
  if (abs(vel) > 0)
  {
    ...
    Real upper = hotBoundary * 100.0; // make this sufficiently large that we
                                      // should definitely capture hot boundary

    pout() << setiosflags(ios::scientific) << setprecision(10);
    LOG("Initial thetaInf guess:  " << lower << " < thetaInf < " << upper);

    // Initial guess
    a_parameters.thetaInterface =
        a_parameters.thetaEutectic +
        1; // Strictly: ( a_parameters.lewis * a_parameters.ThetaInf -
           // a_parameters.thetaInf) / (a_parameters.lewis -1 );

    Real tolerance = 1e-8;         // don't make this any smaller of we'll be
                                   // inconsistent with double vs single precision
    Real hGuess, hotBoundaryGuess; // halfway, thetaInfGuess,
    hotBoundaryGuess = 0;

    int counter = 0;

    while (sqrt(abs(pow(thetaBottom - hotBoundaryGuess, 2))) > tolerance &&
           counter < 100)
    {
      ...
      hGuess = a_parameters.directionalSolidificationMushyZ(
          a_parameters.thetaInterface, zEutectic);

      // Find the predicted value at the hot boundary for this theta_infinity
      hotBoundaryGuess = a_parameters.thetaInf +
                         (1 - a_parameters.thetaInf) * exp(vel * (0 - hGuess));
    ...
    a_parameters.thetaInterface =
        (-a_parameters.lewis * a_parameters.ThetaInf - a_parameters.thetaInf) /
        (a_parameters.lewis - 1);
    ...
    h = a_parameters.directionalSolidificationMushyZ(
        a_parameters.thetaInterface,
        zEutectic); // position of the mush-liquid interface

    LOG("Calculating analytic solution for solidification without flow: ");
    LOG("zEutectic was set to " << zEutectic);
    LOG("h = " << h << ", thetaInf = " << a_parameters.thetaInf
               << ", thetaInterface = " << a_parameters.thetaInterface);
    ...
    for (int i = numPoints; i >= 0; i--)
    {
      ...
      theta = a_parameters.thetaEutectic +
              deltaTheta * ((float)i / (float)numPoints);
  ...
  for (DataIterator dit = enthalpyAnalytic.dataIterator(); dit.ok(); ++dit)
  {
    ...
    for (bit.begin(); bit.ok(); ++bit)
    {
      ...
      else
      {
        ...
        {
          // We're in the liquid
          theta = a_parameters.thetaInf +
                  (a_parameters.thetaInterface - a_parameters.thetaInf) *
                      exp(vel * (z - h));
        ...
        else if (z >= zEutectic)
        {
          // Liquid phase
          solidFraction = 1;
          Real linearTgradient = (thetaTop - a_parameters.thetaEutectic) /
                                 (a_domainLength - zEutectic);
          theta =
              a_parameters.thetaEutectic + linearTgradient * (z - zEutectic);
        }
        else if (abs(vel) > 0)
        {
        ...
        enthalpyAnalytic[dit](iv) =
            (1 - solidFraction) * a_parameters.stefan + theta;
        bulkConcentrationAnalytic[dit](iv) =
            a_parameters.bcValBulkConcentrationLo[1];
      }
