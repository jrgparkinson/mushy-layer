...
void CoefficientInterpolatorLinearFace::define(
    RefCountedPtr<LevelData<FluxBox>> a_coefOld,
    RefCountedPtr<LevelData<FluxBox>> a_coefNew, Real a_timeOld,
    Real a_timeNew) {
  m_coefOld = a_coefOld;
  m_coefNew = a_coefNew;
  m_timeOld = a_timeOld;
...
void CoefficientInterpolatorLinearFace::interpolate(
    LevelData<FluxBox> &a_result, Real a_time) {
  ...
  if (alpha < 0 or alpha > 1) {
    MayDay::Error(
        "CoefficientInterpolatorLinearFace - alpha not between 0 and 1");
  }
...
void CoefficientInterpolatorLinearFace::interpolate(
    LevelData<FluxBox> &a_result, const LevelData<FluxBox> &a_solution,
    Real a_time) {
...
void CoefficientInterpolatorLinearFace::interpolatePrime(
    LevelData<FluxBox> &a_prime, const LevelData<FluxBox> &a_solution,
    Real a_time) {
