...
LevelAdvect::step(LevelData<FArrayBox>&       a_U,
                  ...
                  const Real&                 a_time,
                  const Real&                 a_dt,
                  const bool                  a_doFRupdates,
                  const bool                  a_fillGhosts)
{
...
void LevelAdvect::getPatchGodunov(PatchGodunov& a_patchGodunov)
{
	a_patchGodunov.define( m_domain,  m_dx,
            m_advectPhysics,
            m_normalPredOrder,
            m_useFourthOrderSlopes,
            m_usePrimLimiting,
  ...
  for (dit.begin(); dit.ok(); ++dit)
    {
      ...
      // Shrink cellBox by 1 to avoid edges of ghost cells
      Box smallBox(cellBox);
      int numGhost = a_advectionVelocity.ghostVect()[0];
      smallBox.grow(-numGhost);

      FArrayBox cellVel(smallBox, SpaceDim);
      cellVel.setVal(0.0);
      averageVelToCC(cellVel, advVel, smallBox);
      m_advectPhysics->setVelocities(&cellVel, &advVel);
