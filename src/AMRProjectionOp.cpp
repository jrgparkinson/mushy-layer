...
void AMRProjectionOp::prolongIncrement(
...
#pragma omp for
    for (int ibox = 0; ibox < nbox; ibox++)
    {
      ...
      FORT_PROLONG(
          CHF_FRA_SHIFT(phi, iv), CHF_CONST_FRA_SHIFT(coarse, civ),
          CHF_BOX_SHIFT(region, iv),
          CHF_CONST_INT(
              refinement)); // refinement ratio of 2 (as this is multigrid)
    }
  } // end pragma
}

// Factory
AMRProjectionOpFactory::AMRProjectionOpFactory() { setDefaultValues(); }

//-----------------------------------------------------------------------
//  AMR Factory define function
void AMRProjectionOpFactory::define(
    ...
    Vector<RefCountedPtr<LevelData<FluxBox>>> &a_bCoef, int a_averaging_type)
{
                                    ...
                                    int a_averaging_type)
{
  ...
  define(a_coarseDomain, a_grids, a_refRatios, a_coarsedx, a_bc, alpha, aCoef,
         beta, bCoef, a_averaging_type);
}
//-----------------------------------------------------------------------

MGLevelOp<LevelData<FArrayBox>> *
AMRProjectionOpFactory::MGnewOp(const ProblemDomain &a_indexSpace, int a_depth,
                                bool a_homoOnly)
{
  ...
  if (coarsening > 1 &&
      !m_boxes[ref].coarsenable(coarsening * AMRProjectionOp::s_maxCoarse))
  {
    return nullptr;
  }
...
AMRProjectionOpFactory::AMRnewOp(const ProblemDomain &a_indexSpace)
{
  ...
  // Find number of comps from m_bCoef, which should be defined on at least one
  // level
  ...
  else if (ref == m_domains.size() - 1)
  {
    ...
    newOp->define(m_boxes[ref], m_boxes[ref - 1], m_dx[ref],
                  ...
                  m_exchangeCopiers[ref], m_cfregion[ref], nComp);
  }
