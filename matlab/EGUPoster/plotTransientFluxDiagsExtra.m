function plotTransientFluxDiagsExtra
fontsize = 16;
...
posLowest = [0.12 0.1 0.85 0.23]; 
posLower = [0.12 0.4 0.85 0.28];
posUpper = [0.12 0.68 0.85 0.28];
...
Vice = squeeze(Vi2(i, :))*(0.08);
 ...
 t = getTime(squeeze(times2(i, :)));
    ...
    plot(Vice(doPlot==1),Si(doPlot==1), '-', 'Color', colors(i, :))
...
ax.Position = posLowest;

hold off
box on;
xl = xlabel('$V_{ice}$ (m$^3$)'); 

ylabel('$S_{ice}$ (kg m$^{-2}$)');
...
h.Color = 'white';
  ...
  filename = '/home/parkinsonjl/Documents/Talks/TransientSaltFluxes/transientDiags';
