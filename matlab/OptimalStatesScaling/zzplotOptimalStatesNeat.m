...
%     ylabel('$F_O/\mathcal{C}$');
    ...
    for CR_i = 1:length(CR_arr)
        ...
        x = (Rm_arr-Rm_crit(CR_i)).^(3/4) / (St^(1/2)*sqrt(Le));
        x = (Rm_arr-Rm_crit(CR_i)).^(3/4) / (St^(1/2));
   ...
   % xlabel(['$   (Rm_S-\hat{Rm}_S)^{3/4} / [St^{1/2} Le^{1/2}]$']);
    xlabel(['$   (Rm_S-\hat{Rm}_S)^{3/4} / [St^{1/2}]$']);
...
function  CR_leg = plotScaling(ylab, scaling, optimalMetric, Rm_arr, Rm_crit, CR_arr, St, Le, Vf)
...
% Should do something different for ylab=='$F_O/\mathcal{C}$'
    ...
    for CR_i = 1:length(CR_arr)
        ...
        if sum(validMetrics) > 0
            plot(x(validMetrics) , y(validMetrics));
            CR_leg{end+1} = ['$\mathcal{C}=', num2str(CR), '$'];
        end
    end
    hold off;
