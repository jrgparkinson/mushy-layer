...
calculateEnthalpy()
{
	for (int lev=0; lev<=m_finest_level; lev++)
	{
		...
		for (DataIterator dit = m_amrGrids[lev].dataIterator(); dit.ok(); ++dit)
		{
			...
			(*m_scalarNew[m_enthalpy][lev])[dit()].setVal(0);
			(*m_scalarNew[m_enthalpy][lev])[dit()] += (*m_scalarNew[m_porosity][lev])[dit()];
			(*m_scalarNew[m_enthalpy][lev])[dit()].mult(m_parameters.stefan);

			(*m_scalarNew[m_enthalpy][lev])[dit()] += thetaTerm[dit()];
...
calculatePorosity()
{
	...
	ignore.push_back(m_compositionSolid);
...
calculateBulkConcentration()
{
	...
	for (int lev=0; lev<=m_finest_level; lev++)
	{
		...
		for (DataIterator dit = m_amrGrids[lev].dataIterator(); dit.ok(); ++dit)
		{
			...
			(*m_scalarNew[m_composition][lev])[dit()].setVal(0);
			(*m_scalarNew[m_composition][lev])[dit()] += (*m_scalarNew[m_porosity][lev])[dit()];
			(*m_scalarNew[m_composition][lev])[dit()].mult((*m_scalarNew[m_compositionLiquid][lev])[dit()]);

			(*m_scalarNew[m_composition][lev])[dit()] += ThetaSTerm[dit()];
...
updateEnthalpyVariables()
{
	...
	for (int lev=0; lev<=m_finest_level; lev++)
	{
		...
		LevelData<FArrayBox>& enthalpy = *m_scalarNew[m_enthalpy][lev];
		//		LevelData<FArrayBox>& enthalpySolid = *m_scalarNew[m_enthalpySolid][lev];
		//		LevelData<FArrayBox>& enthalpyLiquid = *m_scalarNew[m_enthalpyLiquid][lev];
		//		LevelData<FArrayBox>& enthalpyEutectic = *m_scalarNew[m_enthalpyEutectic][lev];
		LevelData<FArrayBox>& composition = *m_scalarNew[m_composition][lev];
		LevelData<FArrayBox>& theta = *m_scalarNew[m_theta][lev];

		//		LevelData<FArrayBox>& tempSolidus = *m_scalarNew[m_thetaSolidus][lev];
		...
		//		LevelData<FArrayBox>& porosityEutectic = *m_scalarNew[m_porosityEutectic][lev];

		bool calcTemp = std::find(ignoreVars.begin(), ignoreVars.end(), m_theta) == ignoreVars.end();
		bool calcLiquidConc = std::find(ignoreVars.begin(), ignoreVars.end(), m_compositionLiquid) == ignoreVars.end();
		bool calcSolidConc = std::find(ignoreVars.begin(), ignoreVars.end(), m_compositionSolid) == ignoreVars.end();
		bool calcPorosity = std::find(ignoreVars.begin(), ignoreVars.end(), m_porosity) == ignoreVars.end();

		::updateEnthalpyVariables(enthalpy, composition, theta, compositionLiquid, compositionSolid, porosity,m_parameters,
				calcTemp, calcLiquidConc, calcSolidConc, calcPorosity);
...
amrMushyLayer::regrid()
{
	...
	if (m_max_level > 0)
	{
		...
		for (int lev=lbase+1; lev<= new_finest_level; ++lev)
		{
			...
			//			FourthOrderFillPatch fillerScalar(m_amrGrids[lev], //this level
			//															m_amrGrids[lev-1], //coarser level
			//															1, // 1 variable
			//															m_amrDomains[lev-1], //coarse domain
			//															m_refinement_ratios[lev-1], //refinement ratio of the coarser level
			//															m_num_ghost, //interp radius
			//															true); //fixed time
			...
			for (int a_var=0; a_var < m_numVars; a_var++)
			{
				//This fills interior ghost cells
				//				filler.fillInterpPWConstSpace(*m_scalarNew[a_var][lev],
				//						*m_scalarNew[a_var][lev-1],
				//						0, 0, 1);
				//
				//				filler.fillInterpPWConstSpace(*m_scalarOld[a_var][lev],
				//						*m_scalarOld[a_var][lev-1],
				//						0, 0, 1);
...
amrMushyLayer::convergedToSteadyState()
{
	...
	for (int lev=0; lev <=m_finest_level; lev++)
	{
		...
		for (DataIterator dit = m_scalarNew[m_theta][lev]->dataIterator(); dit.ok(); ++dit)
		{
			...
			(*dThetadt[lev])[dit()].setVal(0);
			(*dThetadt[lev])[dit()] += (*m_scalarNew[m_composition][lev])[dit()];
			(*dThetadt[lev])[dit()] -= (*m_scalarOld[m_composition][lev])[dit()];
			(*dThetadt[lev])[dit()] /= m_dt;
		}
	}
